...
| Phase 3（ロジック監査+モック排除） | ✅ **完了済み（2026-05-05）** | [28_api_migration_plan.md](../genzai/28_api_migration_plan.md) Phase 3セクション参照 |
| Phase 4（その他ロジック削減） | 🔴 Supabase移行と同時実施 | 移行時に参照 |
| **旧31番: 全量調査+残タスク** | ✅ **28番に統合済み（2026-05-05削除）** | [28_api_migration_plan.md](../genzai/28_api_migration_plan.md) |

### 12-2. `useServerTable` composable 統合（楽観的更新の一元化）

**背景・問題**: 全7ページで `filteredRows`（ref）+ `pagedRows`（computed）+ `fetchList()`（API再取得）が個別コピペ実装されている。データ更新後に `refreshList()` でAPI再取得するため、ネットワーク遅延分だけテーブル表示更新が遅れる。各ページで楽観的更新を個別追加すると漏れ・不整合・コピペ地獄になる。

**発見経緯**: MF決算月チェック（fiscal-check）実装時に、`updateClientLocal()` で `clients` refを更新してもテーブル表示用の `filteredRows` refに伝播せず、テーブルが即時更新されない問題が発覚（2026-05-19）。

**解決策**: `useServerTable<T>` composable を新設し、全7ページの共通ロジックを統合。

```typescript
// composables/useServerTable.ts
function useServerTable<T>(options: {
  fetchFn: (query: ServerTableQuery) => Promise<{ rows: T[], totalCount: number }>
  idKey: keyof T  // 'clientId' | 'vendor_id' | 'leadId' 等
  updateFn?: (id: string, patch: Partial<T>) => Promise<void>
  deleteFn?: (id: string) => Promise<void>
}) {
  const rows = ref<T[]>([])           // filteredRows 相当
  const isLoading = ref(true)
  const currentPage = ref(1)
  const totalCount = ref(0)
  const totalPages = ref(1)

  // API取得（ソート・フィルタ・ページネーション対応）
  async function fetchList(query?: ServerTableQuery) { ... }

  // 楽観的更新（即時反映 + API保存 + 失敗ロールバック）
  async function updateRow(id: string, patch: Partial<T>) {
    const idx = rows.value.findIndex(r => r[options.idKey] === id)
    if (idx < 0) return
    const backup = { ...rows.value[idx]! }
    rows.value[idx] = { ...backup, ...patch }  // 即時反映
    try {
      await options.updateFn?.(id, patch)      // API保存
    } catch {
      rows.value[idx] = backup                 // ロールバック
      throw err
    }
  }

  // 行追加・削除も同パターン
  async function addRow(item: T) { ... }
  async function deleteRow(id: string) { ... }

  // ページ変更
  function goToPage(page: number) { ... }

  return { rows, isLoading, currentPage, totalCount, totalPages, fetchList, updateRow, addRow, deleteRow, goToPage }
}
```

**対象ページ（全7ページ）:**

| # | ページ | テーブル用ref | idKey | 状態 |
|---|---|---|---|---|
| 1 | `MockMasterClientsPage.vue` | `filteredRows` (ref) | `clientId` | ❌ 個別実装 |
| 2 | `MockMasterStaffPage.vue` | `filteredRows` (ref) | `staffId` | ❌ 個別実装 |
| 3 | `MockMasterVendorsPage.vue` | `filteredRows` (ref) | `vendor_id` | ❌ 個別実装 |
| 4 | `MockMasterNonVendorPage.vue` | `filteredRows` (ref) | `vendor_id` | ❌ 個別実装 |
| 5 | `LeadListPage.vue` | `filteredRows` (ref) | `leadId` | ❌ 個別実装 |
| 6 | `MockExportPage.vue` | `pagedRows` (ref) | `id` | ❌ 個別実装 |
| 7 | `MockExportDetailPage.vue` | `pagedRows` (ref) | `id` | ❌ 個別実装 |

**Supabase移行時の効果:**
- `fetchFn` を Supabase クエリに差し替えるだけで全7ページが移行完了
- Supabase Realtime との統合ポイントも `useServerTable` 内に1箇所で集約可能
- 楽観的更新ロジックの漏れ・不整合がゼロになる

**実施タイミング:** Supabase移行前に実施可能（現行JSON API + fetchでも動作する設計）。移行前に統合しておくことで、移行時の作業量を大幅削減。
