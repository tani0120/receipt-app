...
Phase A-0  ★開発環境整備（Docker + CLI + init + start） ← 2026-07-01 追加
     ↓  検証環境が整う。supabase db reset / gen types が実行可能になる
Phase A    DB基盤（テーブル作成 + seed + マイグレーションSQL）
     ↓  データの器ができる。reset + seed + gen typesで検証完了
...
| romaji\_\*.json                | scripts/romaji_benchmark出力                        | 0件（scripts/のみ） | 開発ツール出力。移行対象外       |
...
### Phase Aの実行順序（2026-07-01更新）

```
A-0    ★開発環境整備（Docker + CLI + init + start）← 2026-07-01 追加
  ↓    検証環境が整う
A-1    既存SQL 5本のレビュー
...
A-4    seedスクリプト
  ↓
A-5    clientId発番方式（UUID移行）→ ❌ 不要（取り下げ）
  ↓
A-6    ★環境検証（db reset + seed + gen types）← 2026-07-01 追加
  ↓    A-0で整備した環境で、A-1〜A-4の成果物を実際に検証
```
...
### A-0: 開発環境整備 ★2026-07-01 追加

> **背景**: Phase A-1〜A-5で17本のマイグレーションSQLとseedスクリプトを作成したが、
> 検証環境が整っていないため `supabase db reset`（全SQL適用テスト）と
> `supabase gen types`（型自動生成）が実行できない状態だった。
>
> **発覚した問題（2026-07-01）:**
>
> - Supabase CLI: ローカル未インストール（`npx supabase` で毎回ダウンロード）
> - Docker: 未インストール（`supabase start` / `db reset` に必要）
> - WSL2: 未インストール（Docker Desktop on Windowsの前提）
> - `supabase init`: 未実行（`config.toml` なし）
> - `supabase link`: 未実行（リモートProjectとの接続なし）
>
> **教訓**: SQLを書く前に検証環境を整えるべきだった。
> 「Phase B開始時に検証」と先送りしたのは設計の欠陥。
> Phase Aの完了条件に含まれている以上、Phase Aで検証すべきだった。

#### 手順（10ステップ）

| #    | 作業                                            | 検証内容                                                                                           | 状態 |
| ---- | ----------------------------------------------- | -------------------------------------------------------------------------------------------------- | :--: |
| 0-1  | **Docker Desktop導入**                          | インストーラがWSL2も案内。再起動必要                                                               |  ⬜  |
| 0-2  | **Supabase CLIをdevDependencyに追加**           | `npm install -D supabase` → バージョン固定                                                         |  ⬜  |
| 0-3  | **`npx supabase init`**                         | `supabase/config.toml` 生成。既存の `migrations/` と `seed.sql` は維持                             |  ⬜  |
| 0-4  | **`npx supabase start`**                        | ローカルPostgres + Studio + GoTrue + Storage起動確認                                               |  ⬜  |
| 0-5  | **`npx supabase migration list`**               | CLIが既存17本のSQLを認識するか確認。連番（001〜017）で認識しない場合はタイムスタンプ形式にリネーム |  ⬜  |
| 0-6  | **`npx supabase db reset`**                     | 全17本のSQL適用。構文エラー・FK依存順序・CHECK制約の整合性を検証                                   |  ⬜  |
| 0-7  | **seed適用確認**                                | `db reset` 時に `supabase/seed.sql`（410KB / 10,427行）が自動適用されるか確認                      |  ⬜  |
| 0-8  | **`npx supabase gen types typescript --local`** | ローカルDBから型自動生成 → `src/types/database.types.ts`                                           |  ⬜  |
| 0-9  | **型の確認**                                    | `vue-tsc --noEmit` でビルドエラーなし。全テーブル・カラムが期待通りか                              |  ⬜  |
| 0-10 | **Gitコミット**                                 | `database.types.ts` + `config.toml` + `package.json`（supabase追加）をコミット                     |  ⬜  |

> [!NOTE]
> **Docker Desktopを選択した理由:**
>
> - Supabase公式がDocker Desktop前提で検証している
> - トラブルシュート情報が圧倒的に多い（`supabase start` / `db reset` / `stop`）
> - 代替（Rancher Desktop / Podman Desktop）はSupabase CLIとの互換性問題の報告あり
> - ライセンス: 個人・小規模企業（従業員250人未満 & 年間売上1千万USD未満）は無料

> [!NOTE]
> **`supabase link`（リモートProject接続）はこの段階では不要。**
> ローカル環境での検証が完了してからリモートに接続する方が安全。
> リモートProject: `cujksbvnzjxbklhofyfu.supabase.co`（既存。開発用として使用予定）

#### A-0 完了条件

- ⬜ Docker Desktop起動確認
- ⬜ `npx supabase start` 成功（ローカル環境起動）
- ⬜ `npx supabase migration list` で17本認識
- ⬜ `npx supabase db reset` エラー0件
- ⬜ seed適用成功（5種マスタデータ / 700件超がローカルDBに入る）
- ⬜ `npx supabase gen types` で `database.types.ts` 生成成功
...
| _(receipts + audit_logs)_                 | _schema.sql（参考）_   |  _未配置_  | 🔴 **要整理2件**（後述） |
...
- ✅ 型安全性修正（IDEエラー3件。mfCsvParser nullチェック + JournalForValidation optional化。asキャスト不使用）
- ⬜ `supabase db reset` でエラー0件 → **A-0（環境整備）完了後に実施**
- ⬜ `supabase gen types` で型生成成功 → **A-0完了後に実施**
- ⬜ seed適用成功（A-0で確認）
...
| **A-0** | **小（Docker + CLI + init + start + 検証）**                                   | **1セッション**             |
| A       | 中（SQL作成・確認 + seed）※Phase 3.7で6テーブル追加                            | ✅ SQL作成完了。A-0検証待ち |
