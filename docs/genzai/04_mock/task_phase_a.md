...
**⑤ Run A完了**。次は結果を見ながら正解データ作成（人間）+ ①先行実装（AI並行可）

### 完了済み（Phase A UIモック）

24列定義。v-for化。ソート。フィルタ4種。背景色4色。⋯ドロップダウン。ゴミ箱。一括操作。バッジ/アイコン6種（学習・証票・警告・クレ払い・軽減・適格）。残り: 取引日・摘要・借方/貸方6列 = **8列** + 証票メモ列。

### 確定した実行順序（2026-02-27決定: 選択肢C）

> **原則**: UIは今進めてよい。ロジックは待て。
> **根拠**: ⑤（Run A）の精度結果（tax精度・account精度・handwritten_flag精度・表記揺れ傾向）が②③④の設計判断材料になるため、設計はテスト後に固定する。

| 順序 | タスク | 担当 | 状態 |
|:---:|--------|:---:|:---:|
| 1 | ⑤ 1-1 Run A実行 | AI | ✅完了（2026-02-28） |
| 2 | ⑤ 0-3 Run A結果を見ながら正解データ作成 | **人間** | 🔄作業中（5/18件入力済） |
| 3 | ① 取引日・摘要列実装 | AI | ⬜（②と並行可） |
| 4 | 証票メモ列実装（①と同系のUI作業） | AI | ⬜ |
| 5 | ⑤ 0-4 採点スクリプト骨組み作成 | AI | ⬜ |
| 6 | ⑤ 2-1〜3-1 採点+合否判定 | AI+人間 | ⬜ |
| 7 | ② tax分離設計確定 | AI | ⬜ |
| 8 | ③ 借方/貸方6列実装 | AI | ⬜ |
| 9 | ④ 顧問先UI | AI | ⬜ |
| 10 | ⑥ 本番移行 | AI | ⬜ |

> **順序変更（2026-02-28決定）**: Run Aを先に実行し、結果を見ながら正解データを作成する方式に変更。
> 理由: ゼロから正解を書くより、Gemini出力を見て修正する方が速い。出力形式≒ground_truth.ts形式のため変換不要。精度傾向も同時に把握できる。

> **方針転換（2026-03-06）**: ストリームド調査の結果、厳格な税務計算不要と判明。過去の⑤Run Aテストは無意味に。
> マスタ定義を先行し、定義に基づいたテストを後から行う方針に変更。
> ⑤正解データ作成はボトルネックではなくなった。詳細は [10_nullable_on_document_plan.md](file:///c:/dev/receipt-app/docs/genzai/10_nullable_on_document_plan.md)「次のタスク（全体ロードマップ）」参照。

**次のタスク** = **K6-K9 スタッフ管理UI（権限設定）が最優先**。①とD7-D10は並行可。
...
- [ ] 証票メモ（memo有無アイコン表示+ソート+ホバーでNON_MEANINGFUL／MEANINGFUL＝メモ内容を表示。未実装）← **①の次に実装予定（2026-02-27決定）**
...
| 成果物 | 設計書（新規 or 既存更新） |
| 完了条件 | ユーザー承認 |
| 依存 | **⑤結果確定後に着手**（技術的依存はないが、⑤の精度結果がtax設計の判断材料のため。2026-02-27決定） |
...
| 依存 | **⑤結果後に着手**。⑤→②→③→④の順（2026-02-27決定） |

---

#### ⑤ Run A テスト実行 + 本番スキーマ確定

> **⚠️ 順序変更（2026-02-27再決定）**: ①先行実装。⑤は人間が0-3を並行入力。⑤結果確定後に②を設計。
> 理由: テスト結果（tax精度・account精度・handwritten_flag精度）を根拠に②③④を設計する方が手戻りが少ない。
...
- [/] 0-3: 正解データ18件作成（`ground_truth.ts`）— 型定義+記入ガイド完了、**5/18件入力済（#1〜#5）、残り13件**。方式変更: Run A結果を見ながら作成（`run_a_review.md`参照）
- [ ] 0-4: 採点スクリプト作成（自動採点S1-S5）
- [x] 1-1: Run A実行 — 18/18件成功、$0.183、avg 18.0秒/件（2026-02-28完了）
- [ ] 2-1: 自動採点 + 人間判定
...
| `docs/genzai/04_mock/run_a_review.md` | Run A結果レビュー（画像+Gemini出力+仕訳候補の並列表示） |
| `src/scripts/test_results/run_a_first/_summary.json` | Run Aサマリー（18件、コスト、トークン統計） |
