...
Supabase移行時にfactories/だけを差し替えればよくなる。
...
git add [修正したファイル] && git commit -m "[内容]" && git push
...
git add src/mocks/types/journal_extensions.ts && git commit -m "fix: 拡張型に[プロパティ名]追加" && git push
```
...
【完了後に必ず実行】
npm run type-check
git add src/mocks/types/journal_phase5_mock.type.ts && git commit -m "fix: [変更内容]" && git push
```
...
【完了後に必ず実行】
npm run type-check
git add src/mocks/factories/journalFactory.ts && git commit -m "feat: [関数名]追加" && git push
```
...
| Phase B | Supabase型生成・マッピング層作成 | ⬜ 未着手 |
