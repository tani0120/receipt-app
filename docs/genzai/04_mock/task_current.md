...
- [ ] ⋯ボタン実装（🗑列を⋯列に置換。ホバーで展開、クリックで固定。未読/既読・対象/対象外の並列濃淡トグル＋コピー＋ゴミ箱。exported行は対象外・ゴミ箱を制限）
  ...
  - [ ] ⋯ドロップダウンUI実装（並列濃淡トグル＋即実行ボタン）
  - [ ] 一括操作バー実装（選列チェック時にL4バーをフィルタ→アクションに切替、Gmail式）
  - [ ] コピーロジック（★コピー+摘要、日付維持、画像維持、is_read=false、status=null、labels=[]）
  - [ ] 確認ダイアログ（コピー・出力対象外・ゴミ箱の3操作）
  - [ ] exported行制限（「出力済みは出力対象外、ゴミ箱に移動できません」メッセージ）
- [ ] L4バー一括操作モード（選列チェック時にフィルタ表示→アクション表示にGmail式切替）
  - チェックあり: [未読][既読]並列濃淡 / [対象外][対象]普通ボタン / [コピー][ゴミ箱]普通ボタン + 「X件選択中」常時表示
  - チェックなし: 元のフィルタ表示に復帰（☑未出力 ☐出力済 ☐出力対象外 + 凡例）
  - 初回チェック時ヘルプ「✓をすべて外すと元の画面に戻ります」fadeOut表示
  - 確認ダイアログ（コピー・出力対象外・ゴミ箱の3操作で「X件〇〇しますか？」）
  - 冪等処理（同じ状態への重ね掛けはエラーにしない）
...
2. **定義B（2026-02-14確定、2026-02-20追記）**: status = exported + null。labels = 21種類→Phase Cで20種類（EXPORT_EXCLUDE廃止）。背景色 = is_readのみ（2色）。出力状態は出力列で表示。export_excludeはカラム管理（ラベルではない）。
...
- 背景色（3色・優先順位制）: exported→灰(最優先) > 未読→黄 > 既読→白。出力済みは未読より優先
- 出力状態: 背景色（灰）で表示。加えて新しい出力列でも表示予定（v-for化完了後に実装）
- 出力列のUI詳細: 未定（アイコン/✓/テキスト、位置は出力列実装時に決定）
- フィルタリング: チェックボックス3個（未出力を表示✓/出力済を表示/出力対象外を表示）実装済み
