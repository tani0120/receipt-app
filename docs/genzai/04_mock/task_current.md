...
### H. Pipeline層正式統合（Phase A-2 → API統合前提）

**方針**: `src/scripts/` は将来APIサーバーに統合する分類パイプラインのコアロジック。「実験コード」ではなく「ドメイン中核」として扱う。

#### 残留負債（SDK更新待ち）

- [ ] H1: `classify_test.ts` L218 `usage as Record` キャスト除去（SDK型にthoughtsTokenCountなし。SDK更新後に対応）
- [ ] H2: `gaxios` パッケージ型エラー（`node_modules/gaxios/build/src/gaxios.d.ts` TS18028: Private identifiers requires ES2015+。P1の`tsconfig.pipeline.json`で`skipLibCheck: true`設定により解消見込み）
- [x] H3: PRICING定数虚偽値（P0.2で修正済み。入力$0.075→$0.30(4倍)、出力$0.30→$2.50(8.3倍)、思考$0.35→$2.50(7.1倍)。v1テスト結果のコスト見積りは全て信用不可）

> H2（tsconfig対象外）は事実誤認。実際は `tsconfig.app.json` の `src/**/*` に含まれていたが、DOM用lib（ES2020+DOM）の下にあった。「対象外」ではなく「間違った設定の下にいた」。pipeline統合で解消。
> H3（sharp CJS/ESM）はNodeNext設定で解消見込み。pipeline統合で検証。

#### 統合タスク

- [/] P0: UI列 ↔ DBカラム ↔ パイプラインフィールド対応表作成（ギャップ特定: sub_account/tax_category不足、学習列は層C）
- [/] P0.1: クレ払い列追加（is_credit_card_payment: boolean）— 設計書5ファイル+コード4ファイル更新（2026-02-23）
- [x] P0.2: v3.1確定スキーマ完全対応（2026-02-23）— 3ファイル修正:
  - classify_schema.ts: TaxCategory型(8値enum)新設、G1(sub_account)、G2(tax_category)追加、A30(is_credit_card_payment)追加、PostProcessResult.labels[]追加、ClassifyResult.cost_breakdown追加
  - classify_postprocess.ts: PRICING定数修正（入力$0.075→$0.30、出力$0.30→$2.50、思考$0.35→$2.50。旧値は4-8倍の過小評価だった）、generateLabels()新設（14ラベル自動生成）、estimateCost()→CostBreakdown拡張
  - classify_test.ts: プロンプトに税区分8値enum判定テーブル/クレカ払い/補助科目追記。出力にAPI単価ヘッダー/ラベル統計/料金内訳/1枚単価/月間推計追加
- [ ] P1: `tsconfig.pipeline.json` 新規作成（target=ES2022, module=NodeNext, types=["node"]）
- [ ] P2: `tsconfig.json` にreference追加
- [ ] P3: `tsconfig.app.json` に `src/scripts/**/*` をexclude追加
- [ ] P4: `src/scripts/` → `src/pipeline/` リネーム検討（A/Bテスト完了後）
- [ ] P5: 型エラーゼロ確認（`npx tsc -p tsconfig.pipeline.json --noEmit`）
- [ ] P6: DOM依存がないことを確認（window/File/Blob/fetch等）
