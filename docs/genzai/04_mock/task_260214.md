...
**設計の重要ポイント（定義B準拠）**:
1. **statusは排他的**: 協力型フロー（ENUM 5つ）
   - ❌ 廃止: draft, submitted, needs_info, approved（承認フロー型）
   - ✅ 採用: pending, help, soudan, kakunin, exported（協力型フロー）
2. **labelsは非排他的**: 警告・分類タグ（配列 17個）
   - 9個 → 17個に拡張（計画的拡張）
...
- [x] UI要素から概念的な情報を抽出
  - [x] 事故フラグ
    ...
    - ✅ 定義B: labels（17個）で管理、severityは削除
  ...
  - [x] サポートアイコン → `status`（help/soudan/kakunin）
- [x] journal_v1との差分分析
- [x] 実装優先度決定（Tier 1-3）
...
- [x] 30件ダミーデータの全体設計
  ...
  - [x] status分布（pending 20件、help 3件、soudan 3件、kakunin 3件、exported 1件）
  - [x] labels分布（全17種類を網羅）
  - [x] 複合仕訳を含める（3件以上）
  - [x] ルール適用済み/適用可能を含める
  - [x] インボイス適格/不適格を含める
...
- [ ] `src/mocks/types/journal_phase5_mock.type.ts`作成
  - [ ] `JournalStatusPhase5`型定義（5種類のENUM）
    - `'pending' | 'help' | 'soudan' | 'kakunin' | 'exported'`
  - [ ] `JournalLabelPhase5`型定義（17種類のラベル、TypeScript union型）
...
- [ ] Week 1タスク（Day 15-17: 事故フラグシステム）
  ...
  - [ ] Day 17: UI実装（フロント）
    ...
    - [ ] サポートアイコン表示（help/soudan/kakunin）
    - [ ] 事故フラグアイコン表示
    - [ ] ソート機能
...
**現在のフォーカス**:
- [x] Phase 5ステップ1（レベル3スクショ分析）完了
- [ ] Phase 5ステップ2（概念モデル作成）開始
...
**最終更新**: 2026-02-14（定義B準拠）  
**次のアクション**: Phase 5ステップ2（概念モデル作成）開始
