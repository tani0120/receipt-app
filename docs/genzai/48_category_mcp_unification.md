...
| # | タスク | ファイル | 詳細 | 状態 |
|---|---|---|---|---|
| 6-A | getClientAccounts()初期化ロジック確認 | [accountMasterStore.ts](file:///c:/dev/receipt-app/src/api/services/accountMasterStore.ts) L342-351 | 初回アクセス時にマスタからクローン（`masterAccounts.map(a => ({...a}))`）。MFフィールドはマスタに存在しないのでクローンされない（正常） | ✅ 確認済み |
| 6-B | syncMasterAccountsToClients()の削除同期追加 | [accountMasterStore.ts](file:///c:/dev/receipt-app/src/api/services/accountMasterStore.ts) L232-281 | 新規追加 + 9フィールド同期。**削除は反映しない**（仕訳参照を壊さないため。意図的） | ✅ 確認済み（P4として認識済み） |
