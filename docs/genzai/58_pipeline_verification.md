...
- **初回検証日**: 2026-06-23
- **再検証日**: 2026-06-24（vue-tsc / vite build 通過確認）
- **検証方法**: コード上での全件突合確認 + 型チェック + ビルド検証
- **検証対象**: JournalPhase5Mock 全フィールド × 全出力ステージ
- **検証結果**: 断絶7件を修正済み、残存断絶なし。残存リスク4件は全て「対応不要」と判定。
...
## H. ビルド検証結果（2026-06-24）

| チェック | コマンド | 結果 |
|---|---|:---:|
| 型チェック | `npx vue-tsc --noEmit` | ✅ エラー0件 |
| ビルド | `npx vite build` | ✅ 7.84秒で成功 |

修正済み7件（F1〜F7）のコード実装をファイル単位で再確認し、全件が正しく反映されていることを確認した。

---

## I. 残存リスク対応判定（2026-06-24）

| # | 項目 | 判定 | 理由 |
|---|---|:---:|---|
| R1 | `invoice_number` 全出力先で未参照 | **対応不要** | MF API `postJournals` スキーマに登録番号フィールドがない。MF側で管理 |
| R2 | `direction`とMCP取引先の注入側 | **対応不要** | MFは仕訳単位で取引先を表示。branch内のどちらのsideに`trade_partner_code`があっても同一取引先として認識される。コード上もL1469-1478で`isIncome`判定により正しく振り分け済み |
| R3 | CSV税額列が常に空 | **対応不要** | MFが税込金額+税区分から税額を自動計算する仕様。MF CSVインポート仕様に準拠 |
| R4 | `source`フィールドがJournalPhase5Mockで`optional` | **対応不要** | 設計通りの判別共用体パターン。`normalizeJournalForUI()`で正規化、`isImportedJournal()`型ガードで判別 |

---

## J. 潜在課題（2026-06-24検出 → 修正済み）

| # | 課題 | 重大度 | ファイル | 状態 |
|---|---|:---:|---|:---:|
| P1 | `direction`の不要な`as`アクセス | 🟡→✅ | `src/utils/exportMfCsv.ts:173` | **修正済み** |

> **P1詳細**: `JournalPhase5Mock`型にはL125で`direction: Direction | null`が正式に定義済みだった。
> `(journal as { direction?: string | null }).direction`を`journal.direction`に修正し、不要な`as`を除去。
> vue-tsc通過確認済み（2026-06-24）。
