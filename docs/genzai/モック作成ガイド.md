...
作成日: 2026-02-16（v2）
更新日: 2026-03-03（v2.1 — 画面別フェーズ管理・stores/ルール緩和反映）
...
src/mocks/types/staff_notes.ts  ← 要対応ステータス型定義（StaffNoteKey, StaffNote, StaffNotes） 2026-02-21追加
src/mocks/types/receipt_mock.type.ts  ← 既存（触らない）
src/mocks/composables/useCurrentUser.ts  ← 現在ユーザー名取得composable 2026-02-21追加
...
> **フェーズ別適用**
>
> | ルール | Phase A（UX探索） | Phase B以降（スキーマ確定後） |
> |---|---|---|
> | 1. as any禁止 | ✅ 適用 | ✅ 適用 |
> | 2. 拡張型を使う | ✅ 適用 | ✅ 適用 |
> | 3. Domain型を通す | ❌ 適用しない | ✅ 適用 |
> | 4. 生成関数化 | ❌ 適用しない | ✅ 適用 |
> | 5. Domain層に触らない | ✅ 適用 | ✅ 適用 |
>
> Phase Aではスキーマが流動的なため、ルール3-4は適用しない。
> 変更速度 > 構造の美しさ。
...
src/shared/           ← 共有型定義
src/stores/           ← Pinia Store（※例外あり）
```

理由: 型安全性の基礎。汚染されると全体が連鎖的に崩壊する。

#### shared 層の補足（2026-03-04追加）

「触らない」には**新規ファイルの追加も含む**。
アーキテクチャ固定フェーズでは「追加 = 構造変更」と同義。

| ✅ shared に置いてよい | ❌ shared に置いてはいけない |
|---|---|
| 純粋型定義（type / interface） | 外部サービス仕様（MF CSV必須項目等） |
| enum | UI表示文字列 |
| 値オブジェクト（副作用なし） | モック専用ロジック |
| 外部依存を持たない純粋関数 | アプリ固有の警告ラベル |
| ドメイン概念そのもの | Presentation / Adapter 層の責務 |

> 判断基準: 「削除してもDomain型の整合性に影響があるか？」→ No なら `src/mocks/definitions/` 等に配置。

#### 例外プロセス

sharedへの追加が真に必要な場合は、以下を**全て**経ること:

1. フェーズ移行宣言（Phase B以降）
2. 影響範囲の明示
3. 人間のレビュー承認
...
画面固有の設定ストア（例: `settingsStore.ts`）は変更可。詳細は `00_モック実装時のルール.md` §2 参照。
...
【コミット手順（厳守: 漏れ防止）】
// Step 1: 全変更をステージング（ファイル名手動列挙は禁止）
git add .

// Step 2: 残りゼロ確認（出力があったら追加漏れ）
git status --short
→ 出力がゼロ行であることを確認。1行でもあればgit addからやり直し
...
- lintの結果
- セキュリティチェック結果
- git status --short の結果（残りゼロであること）
- commitハッシュ
```
...
> **画面別フェーズ管理（2026-03-03追加）**: Phase A完了は画面単位で判定する。
> 例: B画面（仕訳一覧）はPhase B宣言可、S画面（設定）はPhase A継続。
> 詳細は `00_モック実装時のルール.md`「画面別フェーズ管理」参照。
...
| 2026-02-20 | journalColumns.ts作成（JournalColumn型+23列定義） | ✅ 完了 |
| 2026-02-20 | ヘッダーv-for化（pr-[8px]列ずれ修正、ソートバグ2件修正含む） | ✅ 完了 |
| 2026-02-20 | getValue()関数追加 | ✅ 完了 |
| 2026-02-20 | ボディ全体v-for化（全23列、6type分岐、needActionバグ修正） | ✅ 完了 |
| 2026-02-20 | L4上部バーUI刷新（表示条件DD+チェックボックス3個+凡例追加） | ✅ 完了 |
| 2026-02-20 | チェックボックスフィルタリング実装（未出力/出力済/出力対象外） | ✅ 完了 |
| 2026-02-20 | スクロールバー常時表示（overflow-y-scroll、ヘッダーズレ防止） | ✅ 完了 |
| 2026-02-20 | 行背景色ロジック改善（exported→灰>未読→黄>既読→白の3色優先順位制） | ✅ 完了 |\r
| 2026-02-21 | 操作列（⋯）DD実装（未読/既読・対象/対象外トグル+コピー+ゴミ箱/復活） | ✅ 完了 |\r
| 2026-02-21 | ゴミ箱ソフトデリート実装（deleted_at方式でDB設計書と統一） | ✅ 完了 |\r
| 2026-02-21 | フィルタ拡張（3個→4個: +ゴミ箱を表示）+フィルタバグ修正 | ✅ 完了 |\r
| 2026-02-21 | 行背景色4色化（deleted_at→濃灰+白字(最優先) > exported→灰 > 未読→黄 > 既読→白） | ✅ 完了 |
| 2026-02-21 | コピー確認メッセージ修正（「未出力にコピーしますか？」に統一） | ✅ 完了 |
| 2026-02-21 | exported行制限（個別DD: 未読/既読・対象/対象外disabled化。一括: スキップ方式） | ✅ 完了 |
| 2026-02-21 | L3ツールバー新設（ホーム・アップロード・出力・学習・設定の5ボタン。bg-sky-600/白字） | ✅ 完了 |
| 2026-02-21 | useDraggable composable新規作成（src/mocks/composables/useDraggable.ts。方式B: elRef watch、getBoundingClientRect clamp、z-indexグローバルカウンタ、body userSelect制御） | ✅ 完了 |
| 2026-02-21 | ドラッグ可能モーダル実装（画像プレビュー・過去仕訳検索の両モーダルにドラッグハンドル付きヘッダー追加。中央初期配置、画面外clamp） | ✅ 完了 |
| 2026-02-21 | staff_notes型定義新規作成（src/mocks/types/staff_notes.ts: StaffNoteKey/StaffNote/StaffNotes） | ✅ 完了 |
| 2026-02-21 | useCurrentUser composable新規作成（src/mocks/composables/useCurrentUser.ts） | ✅ 完了 |
| 2026-02-21 | コメント列実装（staff_notesベース4項目コメントモーダル。ドラッグ移動可。ヒントテキスト付。ソートstaff_notesベース） | ✅ 完了 |
| 2026-02-21 | 要対応列実装（4FAアイコン: NEED_DOCUMENT/NEED_INFO/REMINDER/NEED_CONSULT。ホバーポップアップ遅延付。ソート重み付け8/4/2/1方式。Chatwork URL表示） | ✅ 完了 |
| 2026-02-21 | 証票メモ列実装（アイコン表示+ホバーテキスト。memoベース） | ✅ 完了 |
| 2026-02-21 | sortKey不整合修正（comment列: status→staff_notes, ソートロジックも同期） | ✅ 完了 |
