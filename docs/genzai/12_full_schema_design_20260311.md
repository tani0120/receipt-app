...
| `warning_details` | JSONB | **追加**（2026-03-27） | 警告ラベルごとの詳細理由。例: `{"DEBIT_CREDIT_MISMATCH": "借方合計5,000 ≠ 貸方合計3,000"}` |
| `warning_dismissals` | TEXT[] | **追加**（2026-03-28） | ユーザーが確認済みとした警告タイプ。syncWarningLabelsCore再計算時にスキップ |
| `export_batch_id` | VARCHAR(20) | **追加**（2026-03-27） | 出力バッチID（CSVダウンロード時に設定。ダウンロード履歴から仕訳を逆引き用） |
...
| 10 | バリデーション警告（2026-03-27追加） | モック層で6種の追加警告を実装（CATEGORY_CONFLICT, TAX_UNKNOWN, DESCRIPTION_UNKNOWN, VOUCHER_TYPE_CONFLICT, TAX_ACCOUNT_MISMATCH, SAME_ACCOUNT_BOTH_SIDES）。`syncWarningLabelsCore()`で計算。`warning_details`にホバー詳細を格納。`warning_dismissals`でユーザー確認済み警告をスキップ |
| 11 | MF CSVエクスポート（2026-03-27追加） | `exportMfCsv.ts`に21列MF仕訳帳CSV生成を実装。複合仕訳N対N展開対応。出力前バリデーション（EXCLUDE_LABELS一元定義）。UTF-8 BOM付き出力。出力バッチID追跡 |
| 12 | useAccountSettingsフォールバック内部化（2026-03-29追加） | scope='client'でデータ取得失敗時、composable内部でマスタデータへフォールバック。呼び出し側のmasterSettings参照・三項演算子フォールバック全廃止。開発品質ルール（`.agent/workflows/code_quality.md`）策定。レガシーScreenS_AccountSettings.vue物理削除 |
