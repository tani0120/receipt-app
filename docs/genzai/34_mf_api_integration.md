...
| 項目 | 仕様 |
|---|---|
| アクセストークン有効期限 | 約1時間（短期間） |
| リフレッシュトークン | アクセストークン再発行に使用。有効期限はサービス側設定による |
| 保管場所 | **サーバーサイドのDB**（フロントには絶対に露出させない） |
| 再認可が必要なケース | リフレッシュトークン失効時、スコープ変更時、ユーザーが権限取消時 |
...
| 1-12 | **MCPサーバー経由のアクセスに方針転換** | ⬜ **次のアクション（P0）** |
| 1-13 | アプリポータルで「クラウド会計・確定申告」権限を付与 | ⬜ 未着手（MCP利用の前提条件） |
...
| 優先度 | アクション | 前提条件 |
|---|---|---|
| **P0** | アプリポータルで「クラウド会計・確定申告」権限をユーザーに付与 | アプリポータルにログイン |
