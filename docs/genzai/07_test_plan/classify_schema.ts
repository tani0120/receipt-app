...
export interface AdjustmentEntry {
  type: string;           // 'withholding_tax' | 'transfer_fee' | 'offset' | 'points_self' | 'points_third' | 'discount_contract' | 'discount_post' | 'advance_pure' | 'advance_incidental'
  amount: number;
  description: string;    // 例: '源泉徴収税'
}
...
export interface GeminiClassifyResponse {
  ...
  total_amount: number | null;      // 契約上の対価総額（控除前）。経理方式に従い出力
...
export interface PostProcessResult {
  ...
  forbidden_account_violation: boolean; // v1追加: 禁止科目違反
  forbidden_account_detail: string;     // v1追加: 禁止科目チェック詳細
  adjustments_detail: string;           // v1追加: adjustments整合チェック詳細
}
...
export const CLASSIFY_RESPONSE_SCHEMA = {
  ...
  properties: {
    ...
    total_amount: {
      type: SchemaType.NUMBER,
      nullable: true,
      description: '契約上の対価総額（値引き反映後、支払・控除前）。事業者の経理方式に従い出力。税込経理→税込金額、税抜経理→税抜本体額。入金額ではない。欄自体が存在しない場合はnull'
    },
    amount_unreadable: {
      type: SchemaType.BOOLEAN,
    ...
    adjustments: {
      ...
      items: {
        ...
        properties: {
          type: {
            type: SchemaType.STRING,
            description: '控除種別（withholding_tax/transfer_fee/offset/points_self/points_third/discount_contract/discount_post/advance_pure/advance_incidental）'
          },
          amount: {
            type: SchemaType.NUMBER,
            description: '控除金額'
          ...
          description: {
            type: SchemaType.STRING,
            description: '控除の説明（例: 源泉徴収税 15,823円）'
          }
        },
        required: ['type', 'amount', 'description']
      },
      description: '控除明細。証票上に源泉徴収・振込手数料・相殺等の控除がある場合に出力。ない場合はnullまたは空配列'
    }
  },
  required: [
    'voucher_type',
