...
import { CONFIGS, type AccountingConfig } from './test_config';  // v1追加
...
// --config 引数で事業者設定を選択（v1追加）
const configIndex = process.argv.indexOf('--config');
const CONFIG_KEY = configIndex !== -1 && process.argv[configIndex + 1]
  ? process.argv[configIndex + 1] as string
  : 'kojin_aoiro_tax_included';
const SELECTED_CONFIG = CONFIGS[CONFIG_KEY];
if (!SELECTED_CONFIG) {
  console.error(`\u274c 不明なconfig: ${CONFIG_KEY}`);
  console.error(`   使用可能: ${Object.keys(CONFIGS).join(', ')}`);
  process.exit(1);
}
...
// ============================================================
// v1 Request Prompt（config依存で動的生成）
// ============================================================
...
function buildRequestPrompt(config: AccountingConfig): string {
  ...
  const forbiddenAccountsDesc = config.accounting_method === 'tax_excluded'
    ? '（税抜経理のため仮払消費税・仮受消費税の使用可）'
    : '仮払消費税（TAX_PAID）、仮受消費税（TAX_RECEIVED）は使用禁止';

  const fiscalStart = `${2025}-01-01`;
  const fiscalEnd = `${2025}-12-31`;

  return `以下の証票を解析し、会計処理用JSONを生成せよ。

## 会計コンテキスト
- **事業者種別**: ${config.business_type === 'kojin' ? '個人事業主' : '法人'}
- **事業者名**: ${config.business_name}
- **会計期間**: ${fiscalStart} 〜 ${fiscalEnd}（暦年）
- **会計ソフト**: MF
- **経理方式**: ${methodLabel[config.accounting_method] ?? config.accounting_method}
- **計上基準**: 期中現金
- **課税方式**: ${config.is_taxable_entity ? '原則課税（課税事業者）' : '免税事業者'}
- **仕入税額控除方式**: 個別対応方式
- **消費税端数処理**: 切捨て
- **インボイス登録**: ${config.is_taxable_entity ? 'あり（適格請求書発行事業者）' : 'なし'}
- **経過措置**: 適用する（免税事業者からの仕入 80%控除）
- **標準決済手段**: 現金（支払方法が不明な場合のデフォルト貸方科目）
- **源泉徴収義務**: なし（社員0名の個人事業主）
- **不動産所得**: あり（住宅貸付 → 非課税売上）
- **部門管理**: あり
- **禁止科目**: ${forbiddenAccountsDesc}
...
- **仕訳ルール**:
  - 宛名チェック: 証票の宛名が事業者名（${config.business_name}）と異なる場合 → is_not_applicable=true、entries=空配列。仕訳を作成しない。
  ...
  - 福利厚生費(WELFARE): 社員0名のため使用不可。MISCELLANEOUS等を使用。
...
| # | 控除種別 | total_amountに含むか | 判定理由 |
|---|---|:---:|---|
| 1 | 源泉徴収 | 含む | 契約対価は変わらない |
| 2 | 振込手数料 | 含む | 支払方法による費用 |
| 3 | 相殺（債権債務） | 含む | 支払方法の一種 |
| 4 | ポイント充当（自社発行） | 含まない | 実質的値引き |
| 5 | ポイント充当（第三者負担） | 含む | 対価は変わらない |
| 6 | 値引き（契約時） | 含まない | 契約対価の減少 |
| 7 | 事後値引き | 含まない | 契約対価の修正 |
| 8 | 立替精算（純粋立替） | 含まない | 対価ではない |
| 9 | 立替精算（実質付随費用） | 含む | 実質的に対価の一部 |
...
証票上に源泉徴収・振込手数料・相殺等の控除がある場合、adjustmentsに明細を出力せよ。
type: withholding_tax / transfer_fee / offset / points_self / points_third / discount_contract / discount_post / advance_pure / advance_incidental

## 消費税経理処理

**現在の設定: ${methodLabel[config.accounting_method] ?? config.accounting_method}**
${forbiddenAccountsDesc}
金額は${config.accounting_method === 'tax_excluded' ? '税抜本体額 + 仮払/仮受を分離して' : '全て税込で'}出力。
...
9. **仕訳生成**: 借方・貸方を配列で。複合仕訳は is_composite_transaction=true。
   **各エントリに必ず tax_category（税区分）を指定すること。**
   sub_account（補助科目）は判定できる場合のみ記入。不明ならnull。
10. **控除明細**: 源泉徴収・振込手数料・相殺等があれば adjustments に出力。
11. **明細展開**: 通帳/カードは line_items、レシートは receipt_items。
12. **手書き判定**: handwritten_flagで判定。角印・受領印・スタンプは手書きに含めない。
13. **複数証票**: 1画像に2枚以上写っている場合 has_multiple_vouchers=true。OCRフィールドは全てnull。仕訳候補も空配列。
14. **除外判定時の出力ルール**: is_not_applicable=trueの場合でも、OCR抽出結果は全て出力。journal_entry_suggestions のみ空配列。`;
}
...
async function processOneFile(
  ...
  // 3. プロンプト構築（v1: config依存）
  const requestPrompt = buildRequestPrompt(SELECTED_CONFIG);
  ...
  // v1追加: adjustments（控除明細）表示
  const adjTypeLabels: Record<string, string> = {
    'withholding_tax': '源泉徴収', 'transfer_fee': '振込手数料', 'offset': '相殺',
    'points_self': 'ポイント(自社)', 'points_third': 'ポイント(第三者)',
    'discount_contract': '値引き(契約時)', 'discount_post': '事後値引き',
    'advance_pure': '立替(純粋)', 'advance_incidental': '立替(付随)',
  };
  if (geminiResult.adjustments?.length) {
    console.log(`   📋 控除明細(adjustments):`);
    for (const adj of geminiResult.adjustments) {
      const label = adjTypeLabels[adj.type] ?? adj.type;
      console.log(`      ${label}(${adj.type}): ¥${adj.amount.toLocaleString()} (${adj.description})`);
    }
  }
...
async function main() {
  ...
  for (let i = 0; i < rawResults.length; i++) {
    ...
    const postprocess = runPostProcess(raw.gemini, i, allGeminiResults, raw.tokens, SELECTED_CONFIG.accounting_method);  // v1: config依存
    ...
    console.log(`      禁止科目: ${postprocess.forbidden_account_detail}`);
    console.log(`      控除明細: ${postprocess.adjustments_detail}`);
  ...
  console.log(`   禁止科目違反: ${finalResults.filter(r => r.postprocess.forbidden_account_violation).length}件`);  // v1追加
  console.log(`\nラベル統計:`);
