 ...
 *   B9: forbidden_account_violation — 禁止科目チェック（v1追加）
 *   B10: adjustments_detail — adjustments整合チェック（v1追加）
 */
...
interface ForbiddenAccountCheckResult {
    violation: boolean;
    detail: string;
}

/**
 * 経理方式に応じた禁止勘定科目の使用を検出
 * @param accountingMethod - 'tax_included' | 'tax_excluded' | 'exempt'
 */
export function checkForbiddenAccounts(
    r: GeminiClassifyResponse,
    accountingMethod: string
): ForbiddenAccountCheckResult {
    if (!r.journal_entry_suggestions || r.journal_entry_suggestions.length === 0) {
        return { violation: false, detail: '仕訳候補なし' };
    }

    const forbidden: Record<string, string[]> = {
        'tax_included': ['TAX_PAID', 'TAX_RECEIVED'],
        'exempt': ['TAX_PAID', 'TAX_RECEIVED'],
        'tax_excluded': [],  // 税抜では制限なし
    };

    const blockedAccounts = forbidden[accountingMethod] ?? [];
    const violations: string[] = [];

    for (const entry of r.journal_entry_suggestions) {
        if (blockedAccounts.includes(entry.account)) {
            violations.push(`${entry.entry_type}:${entry.account}(¥${entry.amount})`);
        }
    }
    ...
    if (violations.length > 0) {
        return {
            violation: true,
            detail: `⚠️ 禁止科目検出: ${violations.join(', ')}（${accountingMethod}モードで使用不可）`,
        };
    }

    return { violation: false, detail: '✅ 禁止科目なし' };
}

// ============================================================
// B10: adjustments整合チェック（v1追加）
// ============================================================

interface AdjustmentsCheckResult {
    detail: string;
}

export function checkAdjustmentsConsistency(r: GeminiClassifyResponse): AdjustmentsCheckResult {
    const typeLabels: Record<string, string> = {
        'withholding_tax': '源泉徴収',
        'transfer_fee': '振込手数料',
        'offset': '相殺',
        'points_self': 'ポイント(自社)',
        'points_third': 'ポイント(第三者)',
        'discount_contract': '値引き(契約時)',
        'discount_post': '事後値引き',
        'advance_pure': '立替(純粋)',
        'advance_incidental': '立替(付随)',
    };

    if (!r.adjustments || r.adjustments.length === 0) {
        return { detail: '控除明細(adjustments)なし' };
    }

    const adjustmentTotal = r.adjustments.reduce((sum, a) => sum + a.amount, 0);

    return {
        detail: `控除明細(adjustments)合計: ¥${adjustmentTotal}（${r.adjustments.map(a => `${typeLabels[a.type] ?? a.type}(${a.type}):¥${a.amount}`).join(', ')}）`,
    };
}
...
export function runPostProcess(
    ...
    accountingMethod: string = 'tax_included'  // v1追加: config依存
): PostProcessResult {
    ...
    const forbiddenCheck = checkForbiddenAccounts(geminiResult, accountingMethod);  // v1追加
    const adjCheck = checkAdjustmentsConsistency(geminiResult);  // v1追加
    ...
    return {
        ...
        labels,
        forbidden_account_violation: forbiddenCheck.violation,  // v1追加
        forbidden_account_detail: forbiddenCheck.detail,        // v1追加
        adjustments_detail: adjCheck.detail,                    // v1追加
    };
}
