...
| G4 | 簡易課税：勘定科目単位のAI事業区分判定 | ⬜未着手 | 現在は顧問先単位で1つの事業区分（`client.simplifiedTaxCategory`）のみ設定。将来はAIが勘定科目ごとに第1〜6種を推定し、売上科目のデフォルト税区分に事業区分付きID（`SALES_TAXABLE_10_T3`等）を自動設定する機能。MFクラウド会計の勘定科目デフォルト税区分と同等の精度を目指す。インフォバーに「AIで勘定科目単位の事業区分を仮算出しています」と表示予定 |
