...
## 3. MF実データ検証結果（2026-05-23）

> MF側の課税方式を実際に切り替えて全パターンのスナップショットを取得・保存済み。
> 保存先: `data/mf-snapshot-{clientId}-{label}.json`（8ファイル）

### 3-1. 科目と税区分の依存関係

| 項目 | 事業形態（個人/法人）で変わるか | 課税方式で変わるか |
|---|---|---|
| 科目の件数・名前・分類 | ✅ 変わる | ❌ **変わらない** |
| 科目のデフォルト税区分ID | — | ✅ 変わる（売上系のみ。免税→対象外、課税→課税売上10%） |
| 税区分のavailable件数 | ❌ **変わらない** | ✅ 変わる |

### 3-2. 科目件数（課税方式を変えても完全に同一）

| 課税方式 | tsk（個人） | tst（法人） |
|---|---|---|
| 免税 / 簡易 / 一括比例 / 個別対応 | **全て108件** | **全て133件** |

### 3-3. 税区分available件数（個人/法人で完全に同一）

| 課税方式 | available=true | 特徴 |
|---|---|---|
| 免税 | **0件** | 消費税を納めないので全件false |
| 本則（一括比例） | **44件** | 「共通課税仕入」「非課税対応仕入」なし |
| 本則（個別対応） | **73件** | 「共通課税仕入」「非課税対応仕入」あり |
| 簡易課税 | **78件** | 一種〜六種の種別区分あり |

### 3-4. Sugusruマスタ（143件）との差異

| | tsk（個人/免税） | tst（法人/本則） |
|---|---|---|
| 名前一致 | 94件 | 100件 |
| Sugusruのみ | 35件（法人系） | 43件（個人系） |
| MFのみ | 14件（不動産系等） | 46件（法人固有） |

### 3-5. 設計上の影響

1. **科目のavailableフィルタ**: 全科目がavailable=trueなのでフィルタ不要
2. **税区分のavailableフィルタ**: 免税事業者は全件false→フィルタすると0件になる
   - 免税: フィルタなし（全151件使用）
   - 課税: available=trueのみ使用
3. **MFの`tax_method`で課税方式を判定**: `FREE`（免税）/ `GENERAL`（一般）/ `SIMPLIFIED`（簡易）/ `INDIVIDUAL_ALLOCATION`（個別対応）
...
| 2026-05-23 | MF実データ検証結果追加（全課税方式×個人/法人）。科目=事業形態のみ、税区分=課税方式のみで決まることを確認 |
