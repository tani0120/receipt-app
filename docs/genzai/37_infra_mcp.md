...
#### 6. `mfc_ca_getDepartments` — 部門（✅検証済み・TSKで3件取得）

| フィールド    | 型          | 実測値例                                     | 説明               |
| ------------- | ----------- | -------------------------------------------- | ------------------ |
| `id`          | String      | `gw1yDKuG...`                                | 部門ID             |
| `name`        | String      | `テストtks`, `テストtks 子部門`              | 部門名             |
| `parent_id`   | String/null | `null`（ルート）, `gw1yDKuG...`（子部門）    | 親部門ID（木構造） |
| `search_key`  | String      | `""`                                         | 検索キー           |

> 個人事業の場合は部門を使わないケースが多い。TSK（個人事業者）ではテスト用に3件（親1+子2）を設定済み。
> 部門の永続化とUI表示は[54_sub_accounts_departments.md](file:///c:/dev/receipt-app/docs/genzai/54_sub_accounts_departments.md)を参照。
