...
### Python検証環境（converter.py）の位置付け

```
docs/genzai/converter.py     ← リファレンス実装（Python）
docs/genzai/test_converter.py ← テストスイート（7種サンプル、パースエラー0件・貸借一致・税区分UNKNOWN 0件）
data/csv_samples/             ← 実データサンプル
```

- TS側への移植時は `converter.py` のクラス構造（Parser/Exporter）をそのまま踏襲
- 特に「複合仕訳のグループ化」と「freeeタグ→弥生摘要統合ロジック」はそのまま移植すること
