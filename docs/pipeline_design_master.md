...
- ファイル: `src/scripts/previewExtract_test.ts`
...
| `debit_account` / `credit_account` | previewExtract_schema.ts旧世代の残骸。T-P4実測でdirectionで代替確認済み                               |
...
2. 入力型が `GeminiPreviewExtractResponse`（旧設計）のため新パイプライン（LineItem→JournalPhase5Mock）の変換関数として機能しない
...
## DL-006 | VendorVector 66→68種の採用判断（2026-04-04。DL-023で2種追加）
...
> **2026-04-29 統合**: task_unified.md F節（Gemini責務境界）から精度データを統合。以下が正式な精度記録。
...
## ~~DL-015~~ → 📌アーキテクチャ概要（L465）に統合（2026-04-29）

> **統合理由**: DL-015と📌アーキテクチャ概要がほぼ同一内容の重複。📌概要側に以下の独自情報を統合済み。

### DL-015から📌概要に統合した情報

- **入力ファイル判定**: CSV/ExcelはB画面で受付、画像/PDFはA画面→パイプラインへ
- **テストURL**: `http://localhost:5173/#/client/upload/LDI-00008`
- **実装確認**: `isNonJournal()` 関数が `source_type.type.ts` に実装済み
- **level: 'insufficient' の出力方針**: 日付・金額・摘要は確定値、科目はnull（UI赤背景警告）、税区分null
...
> **2026-04-29 注記**: Firebase移行（2026-04-18）後に全ページがスタブ化済み。ScreenE_Workbench.vueはルーターからも外れており完全に死んでいる。旧ページへのルート自体がrouter/index.tsに存在しない可能性あり。
...
## ~~DL-021 | 次フェーズ方針確定（2026-04-05）~~ — 2026-04-29 削除

> **削除理由**: 2026-04-05時点の「次フェーズ」方針。大半が実施済みまたは方針変更済み。最新の方針はtask_unified.mdのCセクションを参照。
> T-P3精度データ（Layer 1-3: 100%/72%/94%）はDL-014およびDL-027に記載済み。
...
> **2026-04-29 注記**: ScreenE関連（Workbench.vue / LogicMaster.vue）は**レガシー・スタブ化済み**。Firebase移行（2026-04-18）でuseJournalEditorが削除され、全関数が空実装。entry=nullで常時ローディング状態。ルーターからも`ScreenE`への参照はgrep 0件。JobStatus型拡張自体は有効。
...
> **2026-04-29 圧縮**: 6提案の詳細議論経緯（Phase A-D、125行）を確定事項テーブルに圧縮。
> 議論の全文が必要な場合はgit履歴を参照。

### 背景

科目推定の全体アーキテクチャについて6つの設計提案が出された。ユーザーが批評→AIが見解→ユーザーが承認の形で採否が確定。
...
| メガベンダーフラグ | ❌ 先送り | 顧問先レベルの科目種類数で代替できる可能性あり。代替案の検証結果次第で再検討 |
| 一意性判定（科目種類数） | ✅ 採用 | FACTベースで健全。80%は初期仮値。実データで調整 |
...
### 核心判断: VendorVector vs ジャンル

- VendorVector = 取引**相手**の属性（何屋か）。マスタ照合で自動取得
- ジャンル = 取引**内容**の属性（何を買ったか）。品目OCRが必要（開発コスト高）
- 差分はわずか1段階の絞り込み。品目OCR開発コストに見合わない → ジャンルは先送り

### コールドスタート解決策

初回は「VendorVector + 自社業種 → 候補提示 → 人間選択」で割り切る。STREAMEDも同方式。2回目以降は学習で自動。
...
┌─────────────────────────────────────┐
│  パイプラインロジック層              │ ← ロジックはここ
│  matchVendor(repo) / previewExtract(repo) │    （matchVendor, previewExtractSourceType,
...
| **Repositoryに入れてはダメなもの** | `matchVendor()`, `previewExtractSourceType()`, `scoreConfidence()`, `determineAccount()` 等のビジネスロジック |
...
│    ↓ POST /api/pipeline/preview-extract （base64画像）              │
└──────────┬──────────────────────────────────────────────────┘
...
| D3  | 前処理（sharp pipeline）を`previewExtract.service.ts`に**直接統合** | `preprocess.ts`はスタンドアロン版。APIではbase64→Buffer→sharp→base64の変換が必要なため別実装 |
...
### PreviewExtract API仕様

```
POST /api/pipeline/preview-extract
...
アップロード時に自動で`/api/pipeline/preview-extract`を呼び出し、結果をバッジ表示：
...
1. `src/scripts/previewExtract_test.ts`でNode.jsスクリプトによるスタンドアロンテストを先行実施（DL-011）
2. UI結合テストのためHonoサーバー＋Vite proxy構成を構築
3. Supabaseクライアント即時初期化がサーバー起動をブロック → Proxy遅延初期化で解消
4. `preprocess.ts`の前処理パイプラインを`previewExtract.service.ts`に統合（base64→Buffer→sharp→base64の変換対応）
...
`logPreviewExtractResult()` でブラウザコンソールに全30項目を構造化出力。
...
| ファイル                                   | 変更内容                                                                  |
| ------------------------------------------ | ------------------------------------------------------------------------- |
| `src/mocks/services/receiptService.ts`     | `AnalyzeOptions`型追加（documentId含む）、`logPreviewExtractResult`全30項目出力 |
| `src/mocks/views/MockUploadPcPage.vue`     | `FileEntry.documentId`追加、`crypto.randomUUID()`生成、previewExtract時に渡す   |
...
## DL-035 | AI分類キーワード外部化 + preview_extract_reason追加（2026-04-12）
...
| 影響ファイル   | `types.ts`（SOURCE_TYPES列挙）、`postprocess.ts`（MODE_MAP追加）、`previewExtract.service.ts`（スキーマ追加） |

#### 2. 分類キーワードの外部ファイル化（source_type_keywords.ts）

**なぜ外部化したか**:

- `previewExtract.service.ts` のSYSTEM_INSTRUCTIONに全12種のキーワードが直書きされていた（約200行）
...
#### 3. preview_extract_reason フィールド追加

| 変更                                  | 内容                                               |
| ------------------------------------- | -------------------------------------------------- |
| `PreviewExtractRawResponse.preview_extract_reason` | AIが判定根拠を自然言語で返すフィールド             |
| `PreviewExtractResponse.preview_extract_reason`    | postprocessでフロントに伝播                        |
...
| ----------------------------------- | ---------------------------------------------------------------- |
| tsc --noEmit                        | エラー0件                                                        |
| journal_voucher判定（仕訳一覧画像） | ✅ 正常判定（preview_extract_reason: 「借方」「貸方」の項目があるため） |
...
| `src/api/services/pipeline/previewExtract.service.ts`     | プロンプトを外部ファイルからの動的結合方式に変更                          |
| `src/api/services/pipeline/types.ts`                | `supplementary_doc`追加、`preview_extract_reason`フィールド追加                  |
| `src/api/services/pipeline/postprocess.ts`          | MODE_MAPにsupplementary_doc追加、preview_extract_reason伝播                      |
| `src/mocks/services/receiptService.ts`              | ログ出力にpreview_extract_reason追加                                             |
...
① アップロード（スマホ / PC）
   ...
   処理: ファイル形式チェック（ホワイトリスト6種） → Gemini previewExtract API
   結果: source_type / direction / confidence / preview_extract_reason
         ↓
...
| ファイル            | 変更内容                                                                                   |
| ------------------- | ------------------------------------------------------------------------------------------ |
| previewExtract.service.ts | thinkingBudget:2048設定、スキーマにdocument_count_reason追加、プロンプトに最優先タスク追加 |
| types.ts            | PreviewExtractRawResponse/PreviewExtractResponseにdocument_count_reason追加                            |
...
- **対象ファイル**: `previewExtract.service.ts`（clearKnownHashes関数）、`pipeline.ts`（DELETEルート）
...
## ~~DL-042~~ → DL-041に統合（2026-04-29）

> **統合理由**: DL-042は8行のみで詳細なし。DL-041（JSON永続化 + API接続）と同セッション（2026-04-19）の実装であり、`task_unified.md` B-12で管理。独立DLとしての記録価値が低い。
...
## DL-048: フェーズ3.5 migrationWorkerにpreviewExtract API統合 + メタデータ永続化全面修正（2026-04-24）

**決定**: migrationWorkerのprocessOneJob()にpreviewExtract API呼び出しを統合。Drive経路のファイルにもAI分類結果を付与し、独自アップロード経路と同等のデータ品質を実現する。

### アーキテクチャ

```
Drive → DL → SHA-256 → previewExtract API → Storage PUT → markJobDone → doc-store書き戻し → ゴミ箱移動
                           ↑
                    excludedはスキップ（コスト削減）
```
...
| ファイル | 変更内容 |
|---|---|
| `migrationWorker.ts` | processOneJob()にpreviewExtract API統合。7ステップフローに拡張。excludedスキップ設計。previewExtract失敗でもジョブは完了（DL+Storageは続行） |
| `documentStore.ts` | `updateAiResults(driveFileId, result, hash)` 新設。PreviewExtractResponse→DocEntryの全20フィールドマッピング。`updateDocumentStatus()`をstatusChangedBy/At/updatedBy/At対応に拡張 |
...
| 判断 | 根拠 |
|---|---|
| excludedはpreviewExtractスキップ | 仕訳外ファイルにAI分類は不要。Vertex AIのAPIコスト削減。将来「仕訳外→仕訳対象に戻す」場合はステータス変更時に個別previewExtractを実行する設計で対応 |
| previewExtract失敗でもジョブはdoneに | DL+Storage PUTは成功しているため、AI結果なしでもファイル自体の移行は完了。手動でpreviewExtractを再実行する手段は将来追加可能 |
...
previewExtract APIで取得してDocEntryに保存済みの15フィールド（aiDate, aiAmount, aiVendor, aiSourceType, aiDirection, aiDescription, aiPreviewExtractReason, aiLineItems, aiLineItemsCount, aiSupplementary, aiDocumentCount, aiWarning, aiProcessingMode, aiFallbackApplied, aiMetrics）が、どのVue画面でも表示されていない。選別画面（`/drive-select/:clientId`）でAI結果を表示すべきだが未実装。task_unified.md L-8で「仕訳一覧UI（C-7）完了後に着手判断」として管理。
...
**なぜ拡張したか**: 独自アップロード経路とDrive経路ではデータフローが根本的に異なる（独自: previewExtract API同期実行、Drive: migrationWorkerで非同期実行）。18列では両経路の区別が消えており、フィールドがどの経路で設定されるかの追跡が不可能だった。
...
## DL-053 | ConfirmedJournal型設計確定 — entries[]構造採用（2026-04-29）

**状態**: 設計確定（T-03実装待ち）

### 決定: JournalPhase5Mock互換のentries[]構造を採用

**なぜフラット構造ではなくentries[]か**:

1. **MF CSVが複合仕訳前提**: 同一取引Noの複数行が1つの仕訳。1行=1仕訳行ではない
2. **UIがentries[]でアクセス**: JournalListLevel3Mock.vueの過去仕訳検索モーダルが `debit_entries[0].account` / `credit_entries[0].tax_category_id` でアクセス
3. **学習ルールとの整合**: LearningRuleEntryLine[]も同じentries[]構造。科目確定パイプライン全体で統一

**根拠（Evidence）**:

- MFテスト用CSV: [テスト仕訳_本則用.csv](file:///c:/dev/receipt-app/docs/genzai/テスト仕訳_本則用.csv)（23列。取引Noでグループ化すれば複合仕訳）
- UI実装: [JournalListLevel3Mock.vue L1820-1863](file:///c:/dev/receipt-app/src/mocks/components/JournalListLevel3Mock.vue#L1820-L1863)（結果テーブル9列: 日付/摘要/借方科目/借方補助/借方税区分/貸方科目/貸方補助/貸方税区分/証憑種別）
- 過去仕訳検索ロジック: [L4986-5061](file:///c:/dev/receipt-app/src/mocks/components/JournalListLevel3Mock.vue#L4986-L5061)（現在localJournals流用。「会計ソフト」タブは`return []`）

**利用用途（全8件）**: 詳細は [25_past_journal.md §2](file:///c:/dev/receipt-app/docs/genzai/25_past_journal.md) 参照

| # | 用途 | 消費箇所URL |
|---|------|------------|
| U-1 | Step 2: 過去仕訳照合 | バックエンド（accountDetermination.ts） |
| U-2 | 一意性判定 | バックエンド |
| U-3 | 過去仕訳検索「会計ソフト」タブ | [仕訳一覧](http://localhost:5173/#/journal-list/TST-00011) |
| U-4 | ヒントモーダル「類似仕訳」 | [仕訳一覧](http://localhost:5173/#/journal-list/TST-00011) |
| U-5 | 学習ルール参考材料 | [学習](http://localhost:5173/#/learning/TST-00011) |
| U-6 | 学習ルール自動提案（将来） | 未実装 |
| U-7 | 科目候補スコアリング（将来） | 未実装 |
| U-8 | MF CSVインポート | [過去仕訳取込](http://localhost:5173/#/history-import/LDI-00008) |

**型定義**: ConfirmedJournal + ConfirmedJournalEntry（2テーブル構成）。詳細は [25_past_journal.md §4](file:///c:/dev/receipt-app/docs/genzai/25_past_journal.md) 参照

**⚠️ → ✅ 解決済み（DL-054）**: snake_case統一を決定。アダプター関数不要。

---

## DL-054 | プロパティ命名snake_case統一方針（2026-04-29）

**状態**: 方針確定（既存camelCaseのリファクタはSupabase移行時）

### 決定: 全パイプライン型をsnake_caseに統一

**現状**:
- **snake_case**: JournalPhase5Mock, JournalEntryLine, Vendor（UI表示系）
- **camelCase**: LearningRule, AccountDeterminationResult（ロジック系）
- → **混在している**

**方針**:
1. **新規型（ConfirmedJournal等）は最初からsnake_case**で作る
2. **既存camelCase型のリファクタはSupabase移行時に一括実施**（対象: LearningRule系 + AccountDeterminationResult）
3. UIが直接使う型はsnake_case必須（アダプター関数を不要にする）

**なぜsnake_caseか**:
- UI（5800行のVue）が `debit_entries[0].account` で直接アクセスしている
- DB（Supabase/PostgreSQL）のカラム名がsnake_case
- 既存データ（vendors_global.ts 224件 / 156KB）がsnake_case
- camelCaseに統一するとUI・DB・データ全てを書き換える必要があり非現実的

**Supabase移行時のリファクタ対象**:

| ファイル | 型 | 現状 | 変更内容 |
|---|---|---|---|
| `learning_rule.type.ts` | LearningRule | camelCase | snake_caseに変換 |
| `learning_rule.type.ts` | LearningRuleEntryLine | camelCase | snake_caseに変換 |
| `accountDetermination.ts` | AccountDeterminationResult | camelCase | snake_caseに変換 |
| `matchLearningRule.ts` | 内部変数 | camelCase | snake_caseに変換 |
