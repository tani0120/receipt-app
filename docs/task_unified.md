...
> 最終更新: 2026-04-25（DL-049: MF CSV実機仕様確定・データ変換Python検証環境整備）
...
### B-19. DL-049: MF CSV実機仕様確定・データ変換Python検証環境整備

| タスク | 内容 | git |
|---|---|---|
| DL-049-1 | `exportMfCsv.ts` 21列→23列（MF実機準拠。借方金額→借方金額(円)、取引先列追加、インボイス税区分直後） | 未コミット |
| DL-049-2 | `converter.py` MFExporter 17列→23列（部門→取引先→税区分→インボイス→金額(円)→税額） | 未コミット |
| DL-049-3 | `converter.py` `to_sjis_safe()` shift_jis→cp932（髙崎栁等IBM拡張文字対応） | 未コミット |
| DL-049-4 | `converter.py` 弥生Exporter: 摘要256文字制限+freeeタグ統合+BOM付UTF-8 | 未コミット |
| DL-049-5 | `converter.py` 部門制限 20→24文字（弥生公式値） | 未コミット |
| DL-049-6 | `test_converter.py` 全テスト合格確認（7種サンプル、パースエラー0・貸借一致・税区分UNKNOWN 0） | 未コミット |
