...
> 最終更新: 2026-04-19（DL-041 JSON永続化+API接続+データ統合パイプライン完了）
...
| タスク | 内容 | git |
|---|---|---|
| DL-040-1 | `DocEntry`型 + `DocumentRepository`型を`repositories/types.ts`に追加 | 未コミット |
| DL-040-2 | `003_documents.sql`新規作成（RLS + インデックス + UNIQUE制約） | 未コミット |
| DL-040-3 | `useDocuments.ts` composable新規作成（モジュールスコープref） | 未コミット |
| DL-040-4 | `documentUtils.ts`算出関数（countUnsorted/latestReceivedDate） | 未コミット |
| DL-040-5 | `useProgress.ts` receivedDate/unsortedをDocEntryから動的算出に改修 | 未コミット |
| DL-040-6 | `MockDriveSelectPage.vue`をuseDocuments接続に改修 | 未コミット |
| DL-040-7 | 進捗管理に「未選別」列追加 + 未出力ハイライト | 未コミット |
| DL-040-8 | ナビバー順序・名称変更（アップロード→資料選別） | 未コミット |

### B-11. DL-041: JSON永続化 + サーバーAPI接続 + データ統合パイプライン

| タスク | 内容 | git |
|---|---|---|
| DL-041-1 | `documentStore.ts`新規作成（インメモリ+JSON永続化ストア） | 未コミット |
| DL-041-2 | `docStore.ts`新規作成（APIルート: GET/POST/PUT/DELETE /api/doc-store） | 未コミット |
| DL-041-3 | `server.ts`にdocStoreルート登録 | 未コミット |
| DL-041-4 | `useDocuments.ts`をAPI接続に改修（ref+サーバーfire-and-forget） | 未コミット |
| DL-041-5 | documentStore.tsの型をrepositories/types.tsからimport（二重定義排除） | 未コミット |
| DL-041-6 | DocEntry型拡張（source: staff-upload/guest-upload、status: supporting、batchId/journalId追加） | 未コミット |
| DL-041-7 | Drive取込→drive-select→ゴミ箱移動の結合テスト完了 | 未コミット |
| DL-041-8 | 独自アップロード→drive-selectの結合テスト完了 | 未コミット |
| DL-041-9 | `.gitignore`にdata/追加（テストデータをgit除外） | 未コミット |
...
| R-S2   | SQLマイグレーション（10テーブル + RLS + trigger + Realtime）                                     | `supabase/migrations/001-003.sql`                                   | ✅ 完了                 | —               |
...
| 24_upload_drive_integration.md | アップロード・ドライブ統合 + ゲスト認証・Drive権限付与 + 資料管理基盤 | 6082ae5a | — | ✅ Drive実装完了 + ゲスト認証UI完了 + DocEntry型/SQL/composable/進捗管理連動完了 + **JSON永続化+API接続完了（DL-041）** + Supabase Auth統合は未着手 |
