...
| タスク     | 内容                                                                                          | git                  |
| ---------- | --------------------------------------------------------------------------------------------- | -------------------- |
| T-00a      | SourceType型(11種) + Direction型(4種) + ProcessingMode型(3種)                                 | 完了                 |
...
### C-0. Repository化（DL-030: データアクセス抽象化）

> **方針（DL-030確定）**: Repositoryはデータの出し入れだけ。ロジックは絶対に入れない。全メソッドPromise<T>で統一。
> 詳細設計: `pipeline_design_master.md` DL-030参照

| タスク | 内容 | 対象ファイル | 状態 | 対応フェーズ |
|---|---|---|---|---|
| R-1 | Repository型定義（5種: Vendor, ClientVendor, IndustryVector, Account, ConfirmedJournal） | `src/repositories/types.ts` | ✅ 完了（2026-04-08 tsc 0件） | **1. UI修正と並行** |
| R-2 | VendorRepository モック実装 | `src/repositories/mock/vendor.repository.mock.ts` | ✅ 完了（2026-04-08 tsc 0件） | 1. UI修正と並行 |
| R-3 | IndustryVectorRepository モック実装 | `src/repositories/mock/industryVector.repository.mock.ts`（新規） | ❌ 未着手 | 4. Step 2-4 |
| R-4 | AccountRepository モック実装 | `src/repositories/mock/account.repository.mock.ts`（新規） | ❌ 未着手 | 4. Step 2-4 |
| R-5 | ConfirmedJournalRepository モック実装 | `src/repositories/mock/confirmedJournal.repository.mock.ts`（新規） | ❌ 未着手 | 4. Step 2-4 |
| R-6 | ClientVendorRepository モック実装 | `src/repositories/mock/clientVendor.repository.mock.ts`（新規） | ❌ 未着手 | 4. Step 2-4 |
| R-7 | Repository集約index + factory関数（VITE_USE_MOCK切り替え） | `src/repositories/index.ts` + `mock/index.ts` | ✅ 完了（2026-04-08 tsc 0件） | R-1〜R-2完了後 |
| R-S | Supabase実装（全5種） | `src/repositories/supabase/`（将来） | ❌ 未着手 | **5. 全モック完了後** |
...
> **方針**: アップロードUI（`/client/upload/:clientId`）をエントリーポイントとし、Step 0から順にGemini接続→精度実測→修正→次ステップの順で進める。
> **テストURL**: `http://localhost:5173/#/client/upload/LDI-00008`
>
> **実行戦略（DL-029: 2026-04-08確定）**: Supabase接続なしでパイプライン精度テストを完結させる。
> - Step 0-1精度テスト: Node.jsスクリプト（`src/scripts/`）でGemini APIを直接呼ぶ
> - UI結合テスト: Vite proxy経由でGeminiを直接呼び出し（Supabase Edge Functions不要）
> - 処理時間: ファイルサイズから計算しモックで擬似体験（DL-011実測: 前処理あり6.3秒/枚。モック遅延5〜8秒に修正済み）
...
|---|---|---|---|---|
| 1 | **UI修正** | モック世界の改善（遅延修正済み、他UI微調整等） | ❌ 未着手 | — |
| 2 | **Step 0-1 精度テスト** | Node.jsスクリプト(`src/scripts/`)による実測（source_type, direction判定） | ❌ 未着手 | 1 完了 |
| 3 | **UI結合テスト** | Vite proxy経由でGeminiを直接呼び出し、アップロードUIからの貫通テスト | ❌ 未着手 | 2 完了 |
| 4 | **Step 2-4 精度テスト** | TSロジック＋Gemini（L4）検証（history_match, vendor_vector等） | ❌ 未着手 | 3 完了 |
...
|---|---|---|---|
| **Step 0** | source_type（11種）判定の精度実測 | — | 2. Step 0-1 |
| **Step 1** | direction（4種）判定の精度実測 | Step 0完了 | 2. Step 0-1 |
...
| Group 1 | Step 0（証票分類）+ Step 1（仕訳方向判定）テスト | ❌ | 2. Step 0-1 |
| Group 2 | Step 2（history_match）テスト — AI不要 | ❌ | 4. Step 2-4 |
| Group 3 | Step 3（業種ベクトル）+ Step 4（科目確定）テスト | ❌ | 4. Step 2-4 |
| E2E | 全5ステップ結合テスト | ❌ | 3. UI結合 または 5以降 |

### C-8. Phase 2 技術的負債解消（C-1の川上テストで必要な順に着手）

| タスク | 内容 | 根拠 | 対応フェーズ |
|---|---|---|---|
| E-2対応 | GeminiVisionService.ts を新パイプラインに全面書き換え | 旧世代コード | 2. Step 0-1 |
| E-5対応 | classify_schema.ts / FileTypeDetector.ts / journal_inference.ts の新設計移行 | 旧世代が新11種と不整合 | 2. Step 0-1 |
...
| コード                  | 世代             | 問題                                                  |
| ----------------------- | ---------------- | ----------------------------------------------------- |
| classify_schema.ts      | 旧               | voucher_type 7種（旧設計）→ 新11種source_typeに未対応 |
| GeminiVisionService.ts  | 旧               | gemini-pro-vision（廃止済み）                         |
| FileTypeDetector.ts     | 旧               | 8種ファイル形式（新11種と不整合）                     |
| journal_inference.ts    | 新（スケルトン） | 287行だが未実装（throw Error）。不要の可能性大        |
| document_filter_test.ts | **新（稼働中）** | 11種+4種                                              |
| source_type.type.ts     | **新（稼働中）** | 11種+ProcessingMode                                   |
