...
## ⚡ 2026-04-18: Firebase → Supabase 完全移行 — ✅ 実施済み

> **検証日**: 2026-04-29（grep実査）
> **検証結果**: src/内に`firebase`文字列を含むTS/Vueファイル0件、firebase SDKからのimport 0件、`VITE_SUPABASE_URL`は`src/lib/supabase.ts`で参照確認

| 項目 | 変更内容 | 検証 |
|---|---|---|
| **認証** | Firebase Auth → Supabase Auth（`src/utils/auth.ts` 書き換え） | ✅ firebase import 0件 |
| **ストレージ** | Firebase Storage → Supabase Storage（`src/api/lib/storage.ts` 書き換え） | ✅ firebase import 0件 |
...
## A. 設計原則 → pipeline_design_master.md に移動予定

> 以下の設計情報はタスクではないため、pipeline_design_master.md 整理時に統合する。
> - TSルールベース vs Gemini の使い分け（優先順位）
> - 型定義前テストの原則
...
### B-19. DL-049: MF CSV実機仕様確定・データ変換Python検証環境整備 → **DL-050で一部廃止**

| タスク | 内容 | git |
|---|---|---|
| DL-049-1 | `exportMfCsv.ts` 21列→23列（MF実機準拠。借方金額→借方金額(円)、取引先列追加、インボイス税区分直後） | 未コミット |
| ~~DL-049-2~~ | ~~`converter.py` MFExporter 17列→23列~~ | **廃止（DL-050）** |
| ~~DL-049-3~~ | ~~`converter.py` `to_sjis_safe()` shift_jis→cp932~~ | **廃止（DL-050）** |
| ~~DL-049-4~~ | ~~`converter.py` 弥生Exporter: 摘要256文字制限+freeeタグ統合+BOM付UTF-8~~ | **廃止（DL-050）** |
| ~~DL-049-5~~ | ~~`converter.py` 部門制限 20→24文字~~ | **廃止（DL-050）** |
| ~~DL-049-6~~ | ~~`test_converter.py` 全テスト合格確認~~ | **廃止（DL-050）** |

> **注意**: DL-049-2〜6はDL-050で「3ソフト変換はMFインポート機能に委ねる」決定により廃止。
> converter.py/test_converter.py等は `docs/genzai/参照資料_税区分変換/` に移動済み。

### B-20. DL-050: 会計マスタ標準化・3ソフト変換方針確定・MF実機テスト

| タスク | 内容 | git |
|---|---|---|
| DL-050-1 | `tax-category-master.ts` 全151件 active:true統一 + effectiveFrom/To修正（輸入消費税額7.8%/6.24%含む） | 未コミット |
| DL-050-2 | MF実機CSV（税区分設定.csv 151件）との完全突合検証 → 全件一致確認 | 未コミット |
| DL-050-3 | 3ソフト方言対応表（tax-category-mapping.ts）廃止 → 参照資料に移動 | 未コミット |
| DL-050-4 | Python変換ロジック（converter.py等17ファイル）廃止 → 参照資料に移動 | 未コミット |
| DL-050-5 | MF実機インポートテスト4パターン実施 → 本則CSV統一出力方針確定 | 未コミット |
| DL-050-6 | 勘定科目差異調査（個人/法人5件のみ。MFマッチング機能で吸収確認） | 未コミット |

### B-21. DL-051: 全108フィールド完全監査
...
| DL-051-6 | 監査レポート field_audit.md v3 作成 | 未コミット |

### B-22. DL-052: typeDefinitionsData.ts 24列化・CellValue記号統一

| タスク | 内容 | git |
|---|---|---|
| DL-052-1 | CellValue型を4記号→6記号に刷新（✅/→/✏️/🔧/⛔/—） | 未コミット |
| DL-052-2 | TypeFieldインターフェースを18列→24列に拡張（独自/Drive経路分離） | 未コミット |
| DL-052-3 | 全108フィールドの値をTSコード根拠で再マッピング | 未コミット |
...
| タスク | 内容 | git |
|---|---|---|
| Step4-C-1 | `accountDetermination.ts` 新規作成（科目確定コア関数。第一層〜第四層フォールバック） | `9341ac7` |
| Step4-C-2 | `matchLearningRule.ts` 新規作成（学習ルール照合関数。exact優先・金額範囲スコア） | `9341ac7` |
...
### C-10. 仕訳UIロジック外出し（JournalListLevel3Mock.vue 5800行 → TS分離）

> **設計書**: [27_journal_list_ui_type.md §外出しファイル構成](file:///c:/dev/receipt-app/docs/genzai/27_journal_list_ui_type.md)
> **方針**: 1ファイルずつ外出し → ビルド確認 → ブラウザ動作確認 → 次のファイル。一括リファクタリング禁止。
> **テストURL**: [仕訳一覧（TST-00011）](http://localhost:5173/#/journal-list/TST-00011)

| タスク | 内容 | 削減行数 | 状態 |
|---|---|---|---|
| R-UI-1 | `journalWarningLabels.ts` 新規作成（warningLabelMap, labelKeyMap, labelTypeLegend定数を外出し） | ~80行 | ❌ |
| R-UI-2 | `accountCategories.ts` 新規作成（SALES/PURCHASE/BS_CATEGORIES, MEGA_GROUPS, VOUCHER_TYPES定数を外出し） | ~40行 | ❌ |
| R-UI-3 | Vue側`syncWarningLabels()`を外出し済み`syncWarningLabelsCore()`呼び出しに統合 + 重複関数200行削除 | ~200行 | ❌ |
| R-UI-4 | `journalTaxRules.ts` 新規作成（isTaxCategoryInvalid, resolveDefaultTaxForClient, computeTaxMismatchSummary） | ~70行 | ❌ |
| R-UI-5 | `journalHintEngine.ts` 新規作成（generateHintValidations, generateHintSuggestions。Extract API移行時にサーバー側移動） | ~300行 | ❌ |

> **問題**: `journalWarningSync.ts`（385行）に外出し済みの`syncWarningLabelsCore()`が存在するが、Vue側に200行の独自実装`syncWarningLabels()`が残存。R-UI-3で統合が最優先。
> **合計削減**: ~690行（5800行 → ~5110行）
...
### ⚠️ E-7: isDuplicateデータ消失（DL-051 T-AUD-5） — 部分解消の可能性

- **実査結果**: `useUpload.ts` の `handleConfirm()` でUploadEntry→DocEntry変換時に `isDuplicate` フラグが消失
- **原因**: DocEntry型（`repositories/types.ts`）に `isDuplicate` プロパティが存在しない
- **影響**: アップロード画面で重複検知した結果が選別画面以降に伝播しない
- **対処**: ①DocEntry型に `isDuplicate: boolean` 追加 ②handleConfirm()で値をコピー
- **2026-04-29 実査**: `repositories/types.ts`にisDuplicateが2件ヒット。**型追加は実施済みの可能性**。handleConfirm側のコピー実装は未確認。要詳細確認
...
## F/G. 設計情報 → pipeline_design_master.md に移動予定

> 以下の設計情報はタスクではないため、pipeline_design_master.md 整理時に統合する。
> - F. Gemini責務境界（source_type 100%, direction 100%, vendor_vector 未テスト, OCR 部分検証）
> - G. 取引先特定4層（T番号100%, 電話72%, 正規化名94%, Gemini88%）
...
| 27_journal_list_ui_type.md | 仕訳一覧UI型定義（46フィールド + EntryLine 8フィールド）+ フィールド責務マトリクス + 警告ラベル15種責務分担 + enum定義 + 外出しファイル構成 | 1eda3a0a | — | ✅ 型定義完了 + 責務マトリクス・外出し計画追記（2026-04-29セッション） |
