...
### 7. ADR-010作成（AI API移行戦略）

**タイトル**: AI API移行戦略（Gemini API → Vertex AI）

**背景**:
- ユーザーからの質問:「firebaseは無料版だが、有料版にしなくてよいのか？」
- 当初の誤った結論:「Gemini APIで無料版で十分」
- UC-186発見後の訂正:「Vertex AI一択 → Blaze Plan（有料版）必要」
...
1. **Phase 1（テスト環境・今すぐ）**: 
   ...
   - Firebase Spark Plan（無料版）維持
   - コスト: $0
   - タスク: AI API実装（Phase 1）- 所要時間2-3時間

2. **Phase 2（本番環境・MVP完成後）**:
   - Vertex AI使用（Cloud Functions経由）
   - Firebase Blaze Plan（有料版）へアップグレード - Cloud Functions必須
   - コスト: 約$12/月（約1,800円）
   - タスク: Firebase Blaze Plan移行 + Vertex AI実装 - 所要時間1日
   - 移行タイミング: 顧問先の実データを扱う時

**作成したファイル**:
1. ADR-010-ai-api-migration.md（目次ファイル）
2. ADR-010-Part1-environment-comparison.md（環境比較詳細）
3. ADR-010-Part2-implementation.md（実装手順・完全なコード）
4. ADR-010-Part3-checklist.md（Phase 1 & 2チェックリスト）
5. ADR-010-Part4-cost-security.md（コスト・セキュリティ）
6. task_ai_api_migration.md（Brain、簡潔なタスク）

**更新したファイル**:
- READING_INDEX.md（ADR-010シリーズ追加）
- SYSTEM_PHILOSOPHY.md（AI モデル記載にADR-010リンク追加）
- walkthrough.md（Brain）
...
**背景**:
- ADR-010作成時の更新漏れ（TASK_MASTER.md、SESSION_20260122.md）
- ユーザーの指摘:「なぜ更新すべきファイルを更新しない？仕組化・ルール化すべきでは？」

**議論**:
- ファイル新規作成 vs 修正の判断基準は？
- ADR作成時に限定する理由は？
- 議論単位で管理すべきでは？

**決定内容**:
1. **議論ライフサイクル管理プロトコル**を確立
   - 議論が起点、成果物は結果
   - Phase 1: 議論発生
   - Phase 2: 議論進行中
   - Phase 3: 議論決定（ADR/Design/実装）
   - Phase 4: 議論未決定（保留）
   - 各フェーズで更新すべきファイルを明確化

2. **session-management-protocol-complete.mdに統合**
   - 1.6. 議論ライフサイクル管理プロトコル追加

**作成したファイル**:
- FAILURE_ANALYSIS_20260122.md（失敗分析）
- FILE_UPDATE_CRITERIA.md（ファイル更新判断基準）
- ADR_CREATION_CHECKLIST.md（ADR作成時チェックリスト）→ 削除予定

**更新したファイル**:
- session-management-protocol-complete.md（議論ライフサイクル管理プロトコル追加）
...
| ファイル | 内容 |
|---------|------|
| ADR-010-ai-api-migration.md | AI API移行戦略（目次） |
| ADR-010-Part1-environment-comparison.md | 環境比較詳細 |
| ADR-010-Part2-implementation.md | 実装手順・完全なコード |
| ADR-010-Part3-checklist.md | Phase 1 & 2チェックリスト |
| ADR-010-Part4-cost-security.md | コスト・セキュリティ |

#### ドキュメント管理プロトコル関連（3件）
| ファイル | 内容 |
|---------|------|
| FAILURE_ANALYSIS_20260122.md | 失敗分析（Gemini API/Vertex API混同） |
| FILE_UPDATE_CRITERIA.md | ファイル更新判断基準 |
| session-management-protocol-complete.md（更新） | 議論ライフサイクル管理プロトコル追加 |
