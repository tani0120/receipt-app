 ...
 * プロンプト定義パネル用データ
 *
 * ★ source_type_keywords.ts（サーバー側実装）と完全一致させること。
 *    サーバー側を変更した場合は必ずこのファイルも同期すること。
 *
...
// ============================================================
// 全12種のキーワード定義（source_type_keywords.ts と完全一致）
// ============================================================
...
export const SOURCE_TYPE_KEYWORDS: SourceTypeKeywords[] = [
  ...
  // ─── 12. other（その他） ───
  {
    ...
    note: '上記11種のいずれにも該当しない。判断できない場合の代替（フォールバック）',
  },
]

// ============================================================
// 紛らわしい境界の判定ガイド（source_type_keywords.ts と完全一致）
// ============================================================

export const BOUNDARY_GUIDES: BoundaryGuide[] = [
  { document: '納税通知書', wrongType: 'tax_payment', correctType: 'supplementary_doc', reason: '受付印・収納印がない。「いくら払え」の通知であり、支払済みの納付書ではない' },
  { document: 'クラウド利用明細（AWS・Google Cloud等）', wrongType: 'credit_card', correctType: 'supplementary_doc', reason: 'カード会社発行ではない。サービス提供者が発行した利用明細' },
  { document: '源泉徴収票', wrongType: 'tax_payment', correctType: 'supplementary_doc', reason: '納付書ではない。年末調整結果の報告書' },
  { document: '納品書（請求書表記なし）', wrongType: 'invoice_received', correctType: 'supplementary_doc', reason: '「請求書」の表記がない。納品確認のみの書類' },
  { document: '見積書', wrongType: 'non_journal', correctType: 'supplementary_doc', reason: '取引情報あり。請求書なしで仕訳根拠になるケースが多い' },
  { document: '残高証明書', wrongType: 'bank_statement', correctType: 'supplementary_doc', reason: '取引明細ではない。特定日の残高のみを証明する書類' },
