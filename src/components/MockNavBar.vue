  ...
  <div>
    ...
    <div class="bg-white border-b border-gray-200 flex items-center justify-between px-3" style="height: 53px; font-family: 'Noto Sans JP', sans-serif">
      ...
      <div class="flex items-center gap-3">
        ...
        >
          <i class="fa-solid fa-gauge-high"></i>{{ UI_MSG.管理者ダッシュボード }}
        </button>
      </div>
      <!-- 右: 管理メニュー（データ駆動） -->
      ...
      <!-- スタッフ切替ドロップダウン -->
        <div class="relative ml-2">
          ...
          <div v-if="showStaffMenu" class="absolute right-0 top-full mt-1 bg-white border border-gray-200 rounded-md shadow-lg z-50 min-w-[200px] py-1">
            <div class="px-3 py-1 text-[10px] text-gray-400 border-b border-gray-100">{{ UI_MSG.ログインスタッフ切替 }}</div>
            <button
...
const topItems: TopItem[] = [
  { key: 'pipeline',   label: UI_MSG.ナビ見込管理,     icon: 'fa-solid fa-chart-line',             path: '/master/leads',               managedPaths: ['/master/leads'] },
  { key: 'clients',    label: UI_MSG.ナビ顧問先管理,   icon: 'fa-solid fa-building',               path: '/master/clients',             managedPaths: ['/master/clients'] },
  { key: 'progress',   label: UI_MSG.ナビ進捗管理,     icon: 'fa-solid fa-bars-progress',          path: '/master/progress',            managedPaths: ['/master/progress'] },
  { key: 'collection', label: UI_MSG.ナビ資料回収,     icon: 'fa-solid fa-folder-open',            path: '/collection',                 managedPaths: ['/collection'] },
  { key: 'conversion', label: UI_MSG.ナビCSV変換,       icon: 'fa-solid fa-arrow-right-arrow-left', path: '/csv-convert',                managedPaths: ['/csv-convert'] },
  { key: 'tasks',      label: UI_MSG.ナビタスク管理,   icon: 'fa-solid fa-list-check',             path: '/task-board',                 managedPaths: ['/task-board'] },
  { key: 'settings',   label: UI_MSG.ナビ設定管理,     icon: 'fa-solid fa-gear',                   path: '/master/settings',            managedPaths: ['/master/settings'] },
];
...
const handleTopClick = (item: TopItem): void => {
  if (item.path) {
    ...
    globalThis.alert(`${item.label}${UI_MSG.未実装です}`);
  }
};
...
const navItems: NavItem[] = [
  ...
  },
  {
    key: 'journal-list',
    label: UI_MSG.ナビ仕訳一覧,
    svgPaths: ['M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01'],
    path: '/journal-list',
  },
  {
    key: 'export',
