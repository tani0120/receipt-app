<template>
  <div class="prompt-root">
    ...
    <p class="prompt-desc">現行のAIプロンプト構成。firstAi API（アップロード/選別AI）と本番AI（Extract API）の2段階。</p>

    <!-- アップロード/選別AI -->
    <section class="prompt-section">
      <div class="prompt-section-header prompt-section-firstAi">
        <i class="fa-solid fa-brain"></i>
        <div>
          <h3>アップロード/選別AI（firstAi API）</h3>
...
行データ（line_items）を抽出してください。</pre>
          ...
          <ul class="prompt-rules">
            ...
            <li><code>first_ai_reason</code> — 判定根拠を日本語1〜2文</li>
    ...
    <section class="prompt-section">
      ...
      <div class="prompt-meta">
        <div class="prompt-meta-item"><span class="prompt-meta-label">用途</span> firstAi確定後、証票画像から本番仕訳データ（勘定科目・税区分等）を抽出</div>
        <div class="prompt-meta-item"><span class="prompt-meta-label">実行タイミング</span> 資料選別OK後。顧問先設定（勘定科目体系・過去仕訳パターン）を参照</div>
        <div class="prompt-meta-item"><span class="prompt-meta-label">ステータス</span> types.tsに型定義のみ。firstAi_test.tsにテスト版プロンプトが存在</div>
        <div class="prompt-meta-item"><span class="prompt-meta-label">ソース</span> <code>src/scripts/firstAi_test.ts</code>（Phase A-2 v2）</div>
      </div>
...
1枚の証票画像（またはCSVテキスト）から、会計処理に必要な全データを1回のレスポンスで抽出します。</pre>
      ...
      <div class="prompt-placeholder">
        ...
        <p class="prompt-placeholder-sub">firstAiの証票種別 + 顧問先マスタの勘定科目体系を動的に注入する設計</p>
      </div>
    </section>
          ...
          <div class="prompt-flow-diagram">
            <div class="flow-step flow-upload">
              ...
              <div class="flow-step-body">
                ...
                <span class="flow-route">独自: POST /api/pipeline/first-ai（即時） / Drive: POST /api/drive/migrate（バッチ）</span>
              </div>
            </div>
            <div class="flow-arrow"><i class="fa-solid fa-arrow-down"></i></div>
            <div class="flow-step flow-firstAi">
              <div class="flow-step-icon"><i class="fa-solid fa-brain"></i></div>
              <div class="flow-step-body">
                <strong>② firstAi API（Vertex AI Gemini）</strong>
            ...
            <div class="flow-step flow-postprocess">
              ...
              <div class="flow-step-body">
                ...
                <span>postprocessFirstAi → validateFirstAiResult → determineAccount（辞書接続）</span>
                <span class="flow-trigger">🔥 発火: ②のVertex AI応答受信直後に同期実行（firstAi関数内部）</span>
              </div>
            </div>
            <div class="flow-arrow"><i class="fa-solid fa-arrow-down"></i></div>
            <div class="flow-step flow-docstore">
              ...
              <div class="flow-step-body">
                ...
                <span class="flow-trigger">🔥 発火: ③完了直後に自動。firstAiのHTTPレスポンスとして返却</span>
              </div>
            </div>
...
.prompt-section-firstAi { background: linear-gradient(135deg, #eff6ff 0%, #f0fdf4 100%); color: #1e40af; }
...
.flow-firstAi .flow-step-icon { background: #fef3c7; color: #d97706; }
