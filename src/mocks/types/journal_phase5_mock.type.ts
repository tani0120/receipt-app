...
export interface JournalPhase5Mock {
   ...
   *   - 実務上99%は伝票日=取引日（中小企業）
   */
  // TODO (2026-04) Supabase: voucher_date→DATE型
  ...
  // N対N複合仕訳（無制限、UI上限15行）
  // TODO (2026-04) Supabase: debit_entries/credit_entries→journal_entriesテーブルに分離（§5.2）
  ...
  // TODO (2026-04) Supabase: deleted_at→TIMESTAMPTZ
  ...
  // TODO (2026-04) Supabase: journal_warning_dismissalsテーブルに分離（journal_id, warning_type, dismissed_by, dismissed_at）
  ...
  // TODO (2026-04) Supabase: export_batch_items(batch_id, journal_id)テーブルに分離
  ...
  // TODO (2026-04) Supabase: memo_created_at→TIMESTAMPTZ
  memo_created_at: string | null;        // メモ作成日時（ISO 8601）

  // スタッフノート（スタッフが自発的に記入するコメント）
  // Phase B TODO (2026-04): journal_staff_notesテーブルに分離
  ...
  // TODO (2026-04) Supabase: created_at→TIMESTAMPTZ DEFAULT NOW(), updated_at→TIMESTAMPTZ DEFAULT NOW()
  // TODO (2026-04) Supabase: status_updated_at/status_updated_by, read_at追加
