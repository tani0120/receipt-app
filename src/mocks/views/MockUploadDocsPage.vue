    ...
    <main class="flex-1 max-w-2xl mx-auto w-full px-3 py-4 pb-40" style="padding-top: 80px;">
      ...
      >
        ...
        >
          ...
          <div class="mt-6 bg-blue-600 text-white text-[14px] font-bold py-3.5 px-8 rounded-2xl shadow-md hover:bg-blue-700 active:scale-95 transition-transform">
            ファイルを選ぶ
          </div>
        </div>
      ...
      <div v-else>
        ...
        <div class="mb-4 bg-white rounded-2xl px-4 py-3 shadow-sm">
          ...
          <div class="flex gap-3 mt-2 text-[10px]">
            ...
            <span v-if="counts.uploading" class="text-amber-500 animate-pulse">⏳ 送信中: {{ counts.uploading }}</span>
        ...
        <div class="space-y-2">
          <div
            ...
            :class="[
              ...
              f.status === 'failed'  ? 'border-red-300'
      ...
      <div v-if="showComplete"
        ...
        <div class="bg-white rounded-3xl p-8 w-full max-w-sm text-center shadow-2xl">
          ...
          <p class="text-[13px] text-gray-500 mb-2">
            <span class="font-bold text-blue-600">{{ confirmedCount }}件</span> の資料を受け付けました。
          </p>
...
const howToItems = [
  ...
  { step: 2, icon: '📤', title: '自動アップロード', desc: 'ファイル選択後に自動で送信が始まります' },
  { step: 3, icon: '✅', title: '全件完了で確定',   desc: '全て送信できたら「確定」ボタンを押してください' },
]
