<template>
  <div class="bg-sky-600 px-3 py-1.5 flex items-center gap-4 text-[11px] tracking-[0.5px] text-white font-semibold" style="font-family: 'Noto Sans JP', sans-serif">
    <button class="flex items-center gap-1 hover:text-sky-100 transition-colors" :class="{ 'text-sky-200 border-b-2 border-white pb-0.5': isActive('home') }">
...
const isActive = (item: string): boolean => {
  ...
  if (item === 'home' && route.path === '/mock/journal-list') return true;
  return false;
};
</script>
