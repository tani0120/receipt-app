...
export type VoucherType =
    | 'RECEIPT'         // 領収書・レシート
    | 'INVOICE'         // 請求書
    | 'TRANSPORT'       // 交通費（ICカード・タクシー等）
    | 'CREDIT_CARD'     // クレジットカード明細
    | 'BANK_STATEMENT'  // 口座取引履歴（通帳等）
    | 'MEDICAL'         // 医療費（薬局・クリニック等）
    | 'NOT_APPLICABLE'; // 仕訳対象外（謄本・名刺・メモ等）
...
export const CLASSIFY_RESPONSE_SCHEMA = {
    ...
    properties: {
        ...
        has_handwritten_memo: {
            type: SchemaType.BOOLEAN,
            description: '手書きメモが存在するか'
        },
        handwritten_memo_content: {
            type: SchemaType.STRING,
            nullable: true,
            description: '手書きメモの内容（読み取れた場合）'
        },
