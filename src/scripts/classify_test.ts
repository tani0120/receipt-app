...
import { runPostProcess, type TokenUsage } from './classify_postprocess';
...
5. **適格請求書**: T番号を検索。ハイフンは除去（T6120002074677形式）。
6. **税率抽出**: 8%/10%の内訳を tax_entries に。複数税率の場合は複数エントリ。
7. **銀行推定**: 通帳の場合、ロゴ・フォーマットから推定。根拠を evidence に。
8. **仕訳生成**: 借方・貸方を配列で。複合仕訳は is_composite_transaction=true。
9. **明細展開**: 通帳/カードは line_items、レシートは receipt_items。
10. **手書きメモ**: 手書き文字があれば内容を記載。
11. **複数証票**: 1画像に2枚以上写っている場合 has_multiple_vouchers=true。`;
...
async function processOneFile(
    ...
    if (geminiResult.journal_entry_suggestions?.length > 0) {
        ...
        for (const e of geminiResult.journal_entry_suggestions) {
            console.log(`      ${e.entry_type === 'debit' ? '借方' : '貸方'}: ${e.account} ¥${e.amount.toLocaleString()} (${e.description})`);
        }
    }
...
async function main() {
    ...
    const totalCost = finalResults.reduce((sum, r) => sum + r.postprocess.estimated_cost_usd, 0);
    ...
    console.log(`\nトークン/料金:`);
    console.log(`   入力合計: ${totalPrompt.toLocaleString()}`);
    console.log(`   出力合計: ${totalCompletion.toLocaleString()}`);
    console.log(`   思考合計: ${totalThinking.toLocaleString()}`);
    console.log(`   合計料金: $${totalCost.toFixed(4)}`);
    console.log(`   平均処理時間: ${(avgDuration / 1000).toFixed(1)}秒`);
