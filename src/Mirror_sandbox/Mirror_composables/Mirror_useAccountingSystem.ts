...
import { FirestoreRepository } from '@/Mirror_sandbox/Mirror_services/Mirror_firestoreRepository';
import { JobService } from '@/Mirror_sandbox/Mirror_services/Mirror_JobService';
import type { Client, Job, SystemSettings, JobStatus, JournalLine, LearningRule, AuditLog } from '@/Mirror_sandbox/Mirror_types/Mirror_firestore';
import { Timestamp } from 'firebase/firestore';
import { Mirror_useBankLogic } from '@/Mirror_sandbox/Mirror_composables/Mirror_useBankLogic';
...
const complexJobIndex = MOCK_JOBS.findIndex(j => j.id === '1002_job01');
if (complexJobIndex !== -1) {
  MOCK_JOBS[complexJobIndex].lines = [
    {
...
export function Mirror_useAccountingSystem() {
  const { identifyBank, generateAutoMaster } = Mirror_useBankLogic();

  // Initialize Data safely
  const initialJobs = MOCK_JOBS;
  ...
  return {
    ...
    subscribeToAllJobs(callback?: (jobs: Job[]) => void) {
      ...
      if (unsubscribeJobs) unsubscribeJobs();
      if (unsubscribeJobs) unsubscribeJobs();
      unsubscribeJobs = FirestoreRepository.subscribeToAllJobs((updated) => {
        // Apply Mapper to EVERY update
        const cleanJobs = updated.map(j => mapJobFromFirestore(j, 'FirestoreSubscription'));
        jobs.value = cleanJobs;
        isLoading.value = false;
