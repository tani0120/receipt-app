    ...
    <header class="shrink-0 z-50 bg-white border-b border-gray-200 shadow-sm relative">
        ...
        <div class="h-12 flex items-center justify-between px-4">
             <div class="flex items-center gap-4 w-full">
                ...
                <!-- Navigation Buttons (12/17 Style) -->
                ...
                <div class="relative ml-4 flex items-center gap-4">
                     <!-- 4. Debug & Controls: Lock Warning (Simulation Toggle) -->
                     <button @click="debugFlags.isLocked = !debugFlags.isLocked" :class="['text-[10px] font-bold px-2 py-1 rounded border', debugFlags.isLocked ? 'bg-yellow-100 text-yellow-800 border-yellow-300' : 'text-gray-400 border-gray-200']">
        ...
        <div class="w-full flex justify-end px-4 py-2 border-t border-gray-100 bg-slate-50 relative overflow-hidden">
             <!-- Lock Warning Banner (Conditional) -->
             <div v-if="debugFlags.isLocked" class="absolute inset-0 bg-yellow-400 flex items-center justify-center z-10 animate-stripe">
                 <span class="text-yellow-900 font-bold text-xs flex items-center gap-2">
                     <i class="fa-solid fa-user-lock"></i> 👨‍💼 他のユーザー(山田 太郎)が現在この仕訳を編集中です。変更は保存されません。
                 </span>
             </div>

             <div v-if="pageMode === 'work'" class="bg-blue-600 text-white px-6 py-1.5 rounded text-sm font-bold shadow-md flex items-center gap-2 transition-all">
                <i class="fa-solid fa-pen-to-square"></i> 🟦 1次仕訳入力モード (未処理: {{ pendingCount }}件)
             </div>
             <div v-else-if="pageMode === 'remand'" class="bg-red-600 text-white px-6 py-1.5 rounded text-sm font-bold shadow-md flex items-center gap-2 animate-pulse transition-all">
                <i class="fa-solid fa-triangle-exclamation"></i> 🟥 ⚠ 差戻し対応モード (残数: {{ pendingCount }}件)
             </div>
             <div v-else-if="pageMode === 'approve'" class="bg-pink-600 text-white px-6 py-1.5 rounded text-sm font-bold shadow-md flex items-center gap-2 transition-all">
    ...
    <main class="flex-1 flex overflow-hidden">
        ...
        <section class="w-[60%] flex flex-col bg-slate-50 border-r border-gray-200 relative z-10">
            <div class="flex-1 overflow-y-auto custom-scrollbar p-4 space-y-4">
                ...
                <!-- 4. Debug Panel (In-UI) -->
                <div class="flex flex-wrap gap-2 mb-2 p-2 bg-gray-100 rounded border border-gray-200">
                    <span class="text-[10px] font-bold text-gray-500 uppercase">検証用フラグ:</span>
                    <label class="text-[10px] flex items-center gap-1 cursor-pointer"><input type="checkbox" v-model="debugFlags.isRemand"> 差戻し</label>
                    <label class="text-[10px] flex items-center gap-1 cursor-pointer"><input type="checkbox" v-model="debugFlags.isDuplicate"> 重複</label>
                    <label class="text-[10px] flex items-center gap-1 cursor-pointer"><input type="checkbox" v-model="debugFlags.isExcluded"> 対象外</label>
                    <label class="text-[10px] flex items-center gap-1 cursor-pointer"><input type="checkbox" v-model="debugFlags.isNew"> 新規</label>
                    <label class="text-[10px] flex items-center gap-1 cursor-pointer"><input type="checkbox" v-model="debugFlags.hasHistory"> 履歴有</label>
                </div>

                <!-- 2. Alert Stack (The "Missing 5") -->
                <div class="space-y-2">
                    <!-- Remand -->
                    <div v-if="debugFlags.isRemand || pageMode === 'remand'" class="bg-red-50 border border-red-200 border-l-4 border-l-red-500 p-3 rounded shadow-sm flex items-start gap-3 animate-pulse">
                        <div class="text-red-600 text-lg"><i class="fa-solid fa-triangle-exclamation"></i></div>
                        <div>
                            <h4 class="font-bold text-xs text-red-800">差戻し (Remand)</h4>
                            <p class="text-[11px] text-red-700 font-bold">⚠ この仕訳は承認者により差し戻されました。内容を確認し、修正してください。</p>
                        </div>
                    </div>
                    <!-- Duplicate -->
                    <div v-if="debugFlags.isDuplicate" class="bg-red-50 border border-red-200 border-l-4 border-l-red-500 p-3 rounded shadow-sm flex items-start gap-3 animate-bounce-short">
                        <div class="text-red-600 text-lg"><i class="fa-solid fa-triangle-exclamation"></i></div>
                        <div>
                            <h4 class="font-bold text-xs text-red-800">重複の可能性 (Duplicate)</h4>
                            <p class="text-[11px] text-red-700 font-bold">重複の可能性！ 同一バッチ内(今回分)に類似データがあります。</p>
                        </div>
                    </div>
                    <!-- Excluded -->
                    <div v-if="debugFlags.isExcluded" class="bg-gray-100 border border-gray-300 border-l-4 border-l-gray-500 p-3 rounded shadow-sm flex items-start gap-3">
                        <div class="text-gray-600 text-lg"><i class="fa-solid fa-ban"></i></div>
                        <div>
                            <h4 class="font-bold text-xs text-gray-700">計算対象外 (Excluded)</h4>
                            <p class="text-[11px] text-gray-600">計算期間対象外としてマークされています（処理対象から除外）。</p>
                        </div>
                    </div>
                    <!-- New Transaction -->
                    <div v-if="debugFlags.isNew" class="bg-purple-50 border border-purple-200 border-l-4 border-l-purple-500 p-3 rounded shadow-sm flex items-start gap-3">
                        <div class="text-purple-600 text-lg"><i class="fa-solid fa-circle-exclamation"></i></div>
                        <div>
                            <h4 class="font-bold text-xs text-purple-800">新規取引 (New Transaction)</h4>
                            <p class="text-[11px] text-purple-700 font-bold">新規取引です。科目が正しいか必ず確認してください。</p>
                        </div>
                    </div>
                    <!-- History Exists -->
                    <div v-if="debugFlags.hasHistory" class="bg-blue-50 border border-blue-200 border-l-4 border-l-blue-500 p-3 rounded shadow-sm flex items-start gap-3">
                        <div class="text-blue-600 text-lg"><i class="fa-solid fa-circle-info"></i></div>
                        <div>
                            <h4 class="font-bold text-xs text-blue-800">実績あり (History Exists)</h4>
                            <p class="text-[11px] text-blue-700">過去に取引実績があります（5回）。AI提案の信頼性が高いです。</p>
                        </div>
                    </div>
                </div>

                <!-- 3. Main Form (Grid Layout) -->
                <div class="bg-white rounded-lg shadow-md border border-slate-300 overflow-hidden">
                <!-- 3. Main Form (Grid Layout) - 12/17 Exact Match -->
                <div class="bg-white border border-slate-300 rounded-lg p-4 shadow-sm space-y-3">
                    ...
                    <div>
                        ...
                        <input type="text" v-model="selectedJobSummary" class="w-full bg-slate-50 border border-slate-300 rounded px-2 py-1.5 text-sm text-slate-800 focus:bg-white transition">
                    </div>
                    ...
                    <div class="grid grid-cols-12 gap-3">
                        <div class="col-span-4">
                            ...
                            <input type="date" v-model="transactionDateStr" class="w-full bg-slate-50 border border-slate-300 rounded px-2 py-1.5 text-sm font-mono">
                        </div>
                        <div class="col-span-4">
                    ...
                    <!-- Debit / Credit Split Area (Height Match) -->
                    <div class="flex gap-4 border-t border-slate-100 pt-3 items-stretch">
                        ...
                        <div class="relative flex-1 flex flex-col">
                            ...
                            <div v-for="(row, idx) in debitRows" :key="'dr-'+idx" class="mb-3 relative group bg-blue-50/30 p-2 rounded border border-blue-100">
                                <button v-if="debitRows.length > 1" @click="removeDebitRow(idx)" class="absolute -right-2 -top-2 bg-white rounded-full border border-gray-200 p-1 text-gray-400 hover:text-red-500 shadow-sm z-10 w-5 h-5 flex items-center justify-center text-[10px]"><i class="fa-solid fa-trash"></i></button>

                                <label class="block text-[10px] text-gray-500 mb-0.5">勘定科目</label>
                                <input type="text" v-model="row.drAccount" class="w-full border border-slate-300 rounded px-2 py-1 text-sm font-bold text-slate-800 mb-2" placeholder="科目">

                                <label class="block text-[10px] text-gray-500 mb-0.5">補助科目</label>
                                <input type="text" v-model="row.drSub" class="w-full border border-slate-300 rounded px-2 py-1 text-xs mb-2" placeholder="(指定なし)">

                                <label class="block text-[10px] text-gray-500 mb-0.5">税区分</label>
                                <select v-model="row.drTaxClass" class="w-full border border-slate-300 rounded px-1 py-1 text-[10px] bg-white mb-2">
                                ...
                                <div class="relative">
                                    <input type="number" v-model.number="row.drAmount" class="w-full border border-slate-300 rounded px-2 py-1 text-sm font-mono text-right bg-white">
                                    <div v-if="row.drTaxClass !== 'tax_none'" class="text-[9px] text-gray-400 text-right mt-1">(内税 {{ Math.floor((Number(row.drAmount)||0) * 10/110).toLocaleString() }})</div>
                                </div>
                            </div>
                            <button @click="addDebitRow" class="text-[10px] text-blue-500 hover:text-blue-700 flex items-center font-bold mt-1"><i class="fa-solid fa-plus-circle mr-1"></i> 行を追加</button>
                        </div>
                        ...
                        <div class="relative flex-1 flex flex-col">
                            ...
                            <div v-for="(row, idx) in creditRows" :key="'cr-'+idx" class="mb-3 relative group bg-green-50/30 p-2 rounded border border-green-100">
                                <button v-if="creditRows.length > 1" @click="removeCreditRow(idx)" class="absolute -left-2 -top-2 bg-white rounded-full border border-gray-200 p-1 text-gray-400 hover:text-red-500 shadow-sm z-10 w-5 h-5 flex items-center justify-center text-[10px]"><i class="fa-solid fa-trash"></i></button>

                                <label class="block text-[10px] text-gray-500 mb-0.5 text-right">勘定科目</label>
                                <input type="text" v-model="row.crAccount" class="w-full border border-slate-300 rounded px-2 py-1 text-sm font-bold text-slate-800 mb-2 text-right" placeholder="科目">

                                <label class="block text-[10px] text-gray-500 mb-0.5 text-right">補助科目</label>
                                <input type="text" v-model="row.crSub" class="w-full border border-slate-300 rounded px-2 py-1 text-xs mb-2 text-right" placeholder="(指定なし)">

                                <label class="block text-[10px] text-gray-500 mb-0.5 text-right">税区分</label>
                                <select v-model="row.crTaxClass" class="w-full border border-slate-300 rounded px-1 py-1 text-[10px] bg-white mb-2">
                                    <option value="tax_none">対象外</option>
                                    <option value="tax_10">課対仕入10%</option>
                                </select>

                                <label class="block text-[10px] text-gray-500 mb-0.5 text-right">金額</label>
                                <input type="number" v-model.number="row.crAmount" class="w-full border border-slate-300 rounded px-2 py-1 text-sm font-mono text-right bg-white">
                            </div>
                            <button @click="addCreditRow" class="text-[10px] text-green-500 hover:text-green-700 flex items-center font-bold justify-end w-full mt-1">行を追加 <i class="fa-solid fa-plus-circle ml-1"></i></button>
                        </div>
                    </div>
                </div>
                </div>

                <!-- AI Analysis & History Panels (Moved Bottom) -->
                <div class="grid grid-cols-2 gap-4 pb-10">
                    ...
                    <div class="bg-white border border-blue-100 rounded-lg shadow-sm overflow-hidden">
                         ...
                         <div class="p-2 space-y-1">
                             <div v-for="i in 3" :key="i" @click="applyAIProposal({ account: '旅費交通費', amount: 1200, summary: 'タクシー代', sub: 'タクシー' })" class="text-[10px] text-gray-600 border-b border-gray-50 pb-1 last:border-0 cursor-pointer hover:bg-blue-50 transition p-1 rounded">
                                 2024/11/{{10-i}}: <span class="font-bold">旅費交通費</span> / 現金 (¥1,200)
                             </div>
                         </div>
                    </div>
                    <!-- AI Analysis Panel (Purple) -->
                    <div class="bg-white border border-purple-100 rounded-lg shadow-sm overflow-hidden">
                         <div class="bg-purple-50 px-3 py-2 border-b border-purple-100 flex justify-between items-center">
                             ...
                             <button @click="applyAIProposal({ account: '旅費交通費', amount: 1200, summary: 'タクシー代', sub: 'タクシー' })" class="bg-purple-600 text-white text-[9px] px-2 py-0.5 rounded shadow hover:bg-purple-700 font-bold">採用する</button>
                         </div>
                         <div class="p-2">
                             <p class="text-[10px] text-gray-600 leading-relaxed">
                                 画像から「タクシー代」と読み取れます。過去の履歴に基づき「旅費交通費」を提案します。
                             </p>
                         </div>
                    </div>
                </div>
            ...
            <!-- Sticky Footer (12/17 Baseline Exact Match with Gray-out Logic) -->
            <div class="bg-white border-t border-gray-200 p-4 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)] z-30 flex gap-3 h-20 items-stretch">
                ...
                <button v-if="pageMode === 'approve'"
                        @click="toggleDecision('remand')"
                        :class="['flex-1 rounded-lg font-bold transition shadow-sm flex items-center justify-center text-lg border', getDecision(selectedJob?.id) === 'remand' ? 'bg-red-700 text-white border-red-800 ring-2 ring-red-400 opacity-100' : (getDecision(selectedJob?.id) ? 'bg-gray-200 text-gray-400 border-gray-300 opacity-50 hover:bg-red-500 hover:text-white hover:opacity-100' : 'bg-red-500 hover:bg-red-600 text-white border-red-600 opacity-90')]">
                    <i v-if="getDecision(selectedJob?.id) === 'remand'" class="fa-solid fa-check mr-2"></i>
                    <i v-else class="fa-solid fa-rotate-left mr-2"></i> 差戻し
                </button>
                <button v-else
                        @click="toggleDecision('unknown')"
                        :class="['flex-1 rounded-lg font-bold transition shadow-sm flex items-center justify-center text-lg border', getDecision(selectedJob?.id) === 'unknown' ? 'bg-yellow-500 text-white border-yellow-600 ring-2 ring-yellow-300 opacity-100' : (getDecision(selectedJob?.id) ? 'bg-gray-200 text-gray-400 border-gray-300 opacity-50 hover:bg-yellow-400 hover:text-yellow-900 hover:opacity-100' : 'bg-yellow-400 hover:bg-yellow-500 text-yellow-900 border-yellow-500 opacity-90')]">
                    <i v-if="getDecision(selectedJob?.id) === 'unknown'" class="fa-solid fa-check mr-2"></i>
                    <i v-else class="fa-solid fa-circle-question mr-2"></i> 不明仕訳あり
                </button>

                <!-- 2. Exclude Button -->
                <button @click="toggleDecision('exclude')"
                        :class="['flex-1 rounded-lg font-bold transition shadow-sm flex items-center justify-center text-lg border', getDecision(selectedJob?.id) === 'exclude' ? 'bg-red-700 text-white border-red-800 ring-2 ring-red-400 opacity-100' : (getDecision(selectedJob?.id) ? 'bg-gray-200 text-gray-400 border-gray-300 opacity-50 hover:bg-red-600 hover:text-white hover:opacity-100' : 'bg-red-600 hover:bg-red-700 text-white border-red-700 opacity-90')]">
                    <i v-if="getDecision(selectedJob?.id) === 'exclude'" class="fa-solid fa-check mr-2"></i>
                    <i v-else class="fa-solid fa-ban mr-2"></i> 仕訳から除外
                </button>
                ...
                <button v-if="pageMode === 'approve'"
                        ...
                        :disabled="!isBalanced"
                        :class="['flex-1 rounded-lg font-bold transition shadow-sm flex items-center justify-center text-lg border', isBalanced ? (getDecision(selectedJob?.id) === 'csv' ? 'bg-blue-800 text-white border-blue-900 ring-2 ring-blue-400 opacity-100' : (getDecision(selectedJob?.id) ? 'bg-gray-200 text-gray-400 border-gray-300 opacity-50 hover:bg-blue-600 hover:text-white hover:opacity-100' : 'bg-blue-600 hover:bg-blue-700 text-white opacity-90')) : 'bg-gray-300 cursor-not-allowed border-gray-300']">
                    <i v-if="getDecision(selectedJob?.id) === 'csv'" class="fa-solid fa-check mr-2"></i>
                    <i v-else class="fa-solid fa-file-csv mr-2"></i> 仕訳CSV化
                </button>
                <button v-else
                        @click="toggleDecision('confirmed')"
                        :disabled="!isBalanced"
                        :class="['flex-1 rounded-lg font-bold transition shadow-sm flex items-center justify-center text-lg border', isBalanced ? (getDecision(selectedJob?.id) === 'confirmed' ? 'bg-blue-800 text-white border-blue-900 ring-2 ring-blue-400 opacity-100' : (getDecision(selectedJob?.id) ? 'bg-gray-200 text-gray-400 border-gray-300 opacity-50 hover:bg-blue-600 hover:text-white hover:opacity-100' : 'bg-blue-600 hover:bg-blue-700 text-white opacity-90')) : 'bg-gray-300 cursor-not-allowed border-gray-300']">
                    <i v-if="getDecision(selectedJob?.id) === 'confirmed'" class="fa-solid fa-check mr-2"></i>
                    <i v-else class="fa-solid fa-check-circle mr-2"></i> 仕訳確定済み
                </button>
            </div>
        </section>
        ...
        <section class="w-[40%] bg-slate-800 relative z-20 flex flex-col">
             ...
             <div class="absolute top-0 left-0 right-0 h-10 bg-black/50 backdrop-blur-sm z-20 flex items-center justify-center gap-2 px-2 shadow-sm">
                 <span class="text-[11px] text-slate-800 font-bold bg-white/90 px-2 py-1 rounded shadow-sm border border-white">{{ client?.settings?.fiscalMonth || 12 }}月決算</span>
                 <span class="text-[11px] text-slate-800 font-bold bg-white/90 px-2 py-1 rounded shadow-sm border border-white">{{ client?.settings?.software || '弥生会計' }}</span>
                 <span class="text-[11px] text-slate-800 font-bold bg-white/90 px-2 py-1 rounded shadow-sm border border-white">{{ client?.settings?.taxMethod || '税抜' }}</span>
                 <span class="text-[11px] text-slate-800 font-bold bg-white/90 px-2 py-1 rounded shadow-sm border border-white">{{ client?.settings?.calcMethod || '発生主義' }}</span>
                 <span class="text-[11px] text-slate-800 font-bold bg-white/90 px-2 py-1 rounded shadow-sm border border-white">{{ client?.settings?.taxType || '青色' }}</span>
             </div>
             ...
             <div class="flex-1 flex items-center justify-center overflow-hidden relative group">
                 ...
                 <img v-if="selectedJob?.imageUrl" :src="selectedJob.imageUrl" class="max-w-full max-h-full object-contain shadow-2xl transition-transform duration-200" alt="Receipt">
...
// NOTE: Use local mocks or specific paths if needed, avoiding main composables if strict isolation required.
// For now, importing from main is allowed in Mirror World Phase 4 as per walkthrough (data injection overrides it).
import { Mirror_useAccountingSystem, type Job, type JournalLine } from '@/Mirror_sandbox/Mirror_composables/Mirror_useAccountingSystem';
...
const { jobs, getClientByCode, updateJob } = Mirror_useAccountingSystem();
...
// Modals & Queue Logic (Restored from 12/17)
...
// Debug Flags
const debugFlags = ref({
    isLocked: false,
    isRemand: false,
    isDuplicate: false, // Default: false
    isExcluded: false,
    isNew: false,
    hasHistory: true // Default: true for demo
});
...
// Determine Mode from Route or Debug
const pageMode = computed(() => (route.query.mode as string) || 'work');

// Form Rows
// Form Rows (Moved below with correct typing)
...
const transactionDateStr = computed({
    get: () => {
        ...
        try { return selectedJob.value.transactionDate.toDate().toISOString().split('T')[0]; }
        catch(e) { return "" }
    },
    set: (v: string) => {
        if (selectedJob.value) selectedJob.value.transactionDate = Timestamp.fromDate(new Date(v));
    }
});
...
watch(selectedJob, (newJob) => {
    if (newJob?.clientCode) client.value = getClientByCode(newJob.clientCode);
});

// -- Methods --
// --- Type Extensions for UI ---
interface EditableJournalLine extends JournalLine {
    drSub?: string;
    crSub?: string;
}
...
// --- 12/17 Restoration Logic: Actions & Queue ---

function getDecision(jobId: string | undefined): string | undefined {
    if (!jobId) return undefined;
    const q = approvalQueue.value.find(x => x.id === jobId);
    return q?.decision;
}
...
function selectJob(index: number) {
    if (index >= 0 && index < jobs.value.length) {
        ...
        // Init Flag Logic based on Job (Mocking for now)
        if (selectedJob.value.status === 'remanded') debugFlags.value.isRemand = true;

        // Reset Rows
        const lines = selectedJob.value.lines && selectedJob.value.lines.length > 0 ? selectedJob.value.lines : [{
            lineNo: 1, drAccount: '', drAmount: 0, drTaxClass: '課対仕入10%', crAccount: '', crAmount: 0, crTaxClass: '対象外', description: ''
        } as JournalLine];

        // Deep copy lines to editable refs
        // Add sub-account fields if missing (for the new UI)
        debitRows.value = lines.map(l => ({ ...l, drSub: '', crSub: '' } as EditableJournalLine));
        creditRows.value = lines.map(l => ({ ...l, drSub: '', crSub: '' } as EditableJournalLine));

        // Extract summary from first line or job desc
        selectedJobSummary.value = lines[0]?.description || '';
    }
}

function addDebitRow() {
    debitRows.value.push({ lineNo: debitRows.value.length + 1, drAccount: '', drSub: '', drAmount: 0, drTaxClass: '課対仕入10%', crAccount: '', crSub: '', crAmount: 0, crTaxClass: '対象外', description: selectedJobSummary.value } as EditableJournalLine);
}
function removeDebitRow(idx: number) { debitRows.value.splice(idx, 1); }

function addCreditRow() {
    creditRows.value.push({ lineNo: creditRows.value.length + 1, drAccount: '', drSub: '', drAmount: 0, drTaxClass: '対象外', crAccount: '', crSub: '', crAmount: 0, crTaxClass: '対象外', description: selectedJobSummary.value } as EditableJournalLine);
}
...
function goNext() {
    if (currentIndex.value < jobs.value.length - 1) {
        ...
        // Loop back to start (Phase 2 Requirement)
        selectJob(0);
        // If at end and done, show completion (Optional, but loop priority)
...
function goBack() {
    if (currentIndex.value > 0) {
        ...
        // Loop to end (Phase 2 Requirement)
        selectJob(jobs.value.length - 1);
    }
}

function applyAIProposal(proposal: any) {
    // Populate Debit Row
    debitRows.value = [{
        lineNo: 1,
        drAccount: proposal.account,
        drSub: proposal.sub || '',
        drAmount: proposal.amount,
        drTaxClass: '課対仕入10%',
        crAccount: '',
        crSub: '',
        crAmount: 0,
    ...
    } as EditableJournalLine];

    // Populate Credit Row (Infer Cash for now or keep generic)
    creditRows.value = [{
        lineNo: 1,
        drAccount: '',
        drSub: '',
        ...
        crAccount: '現金',
        crSub: '',
        crAmount: proposal.amount,
        crTaxClass: '対象外',
        description: proposal.summary
    } as EditableJournalLine];

    selectedJobSummary.value = proposal.summary;
}
...
function toggleDecision(decision: string) {
    ...
    // If decision already made (even if different), we confirm overwrite or revert
    if (existing) {
         if (existing.decision === decision) {
             // Same decision -> toggle off (revert)
             pendingRevertId.value = txId;
             showRevertConfirmModal.value = true;
             return;
         }
         // Different decision? Just overwrite for now (simplified 12/17 logic often just overwrote)
    }
...
function addToQueue(decision: string) {
    ...
    const record: QueueItem = {
        ...
        summary: debitRows.value[0]?.description || '',
        selected: false
    };
...
function confirmRevert() {
    if (pendingRevertId.value) {
        ...
        showCompletionPopup.value = false; // logic from 12/17
    }
}
...
function revertItems() {
    // Logic to revert selected items in batch modal
    ...
    // Go to first available or start
    selectJob(0);
}
...
function executeBatch() {
    ...
    approvalQueue.value = [];
    router.push(`/mirror_jobs/${currentJob.value?.clientCode || '1001'}`); // Go back to Screen B (or A)
}
