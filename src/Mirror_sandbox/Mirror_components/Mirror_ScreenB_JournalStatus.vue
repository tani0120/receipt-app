...
import { Mirror_useAccountingSystem, type Job } from '@/Mirror_sandbox/Mirror_composables/Mirror_useAccountingSystem';

const router = useRouter();
const { jobs, fetchClients } = Mirror_useAccountingSystem();
...
// Transform Real Jobs into "Client Status" Rows compatible with the Template
const filteredClients = computed(() => {
    // Treat each Job as a Row for Strict Spec visualization
    let result = jobs.value.map((job: Job) => {

        let clientName = 'Unknown Client';
        if (job.clientCode === '1001') clientName = '株式会社エーアイシステム';
        else if (job.clientCode === '1002') clientName = '合同会社ベータ企画';
        else if (job.clientCode === 'AMT') clientName = '株式会社 テスト商事';
        else clientName = `Client ${job.clientCode}`;

        // Determine Status based on job.status
        let nextAction = 'work';
        let isNew = false;

        if (job.status === 'ai_processing' || job.status === 'pending') {
            nextAction = 'work';
            isNew = true;
        } else if (job.status === 'remanded') {
            nextAction = 'remand';
        } else if (job.status === 'primary_completed' || job.status === 'waiting_approval') {
            nextAction = 'approve';
        ...
        return {
            id: job.id, // REAL ID for navigation
            name: clientName,
            code: job.clientCode,
            software: job.accountingSoftware || 'freee',
            fiscalMonth: job.fiscalMonth || 3,
            isNew: isNew,
            steps: {
                receipt: 'done',
                aiAnalysis: { state: 'done' }, // Assuming AI done if job exists here
                journalEntry: { state: job.status === 'primary_completed' ? 'done' : 'pending', count: 1 },
                approval: { state: job.status === 'approved' ? 'done' : 'pending', count: 0 },
                remand: { state: job.status === 'remanded' ? 'pending' : 'done', count: 0 },
                export: 'pending',
...
const handleAction = (client: any) => {
    if (['work', 'approve', 'remand'].includes(client.nextAction)) {
        // Navigate to Screen E with REAL ID
        router.push({
            path: `/mirror_journal-entry/${client.id}`,
