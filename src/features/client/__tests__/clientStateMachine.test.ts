...
describe("L3: State Guard - 許可遷移", () => {
    it("Draft → Submitted は許可", () => {
        expect(() => assertClientTransition("Draft", "Submitted")).not.toThrow();
    });

    it("Submitted → Approved は許可", () => {
        expect(() => assertClientTransition("Submitted", "Approved")).not.toThrow();
    });
});

describe("L3: State Guard - 禁止遷移", () => {
    it("Approved → Submitted は禁止（巻き戻し）", () => {
        expect(() => assertClientTransition("Approved", "Submitted")).toThrow(
            StateTransitionError
        );
    });

    it("Approved → Draft は禁止（巻き戻し）", () => {
        expect(() => assertClientTransition("Approved", "Draft")).toThrow(
            StateTransitionError
        );
    });

    it("Submitted → Draft は禁止（巻き戻し）", () => {
        expect(() => assertClientTransition("Submitted", "Draft")).toThrow(
            StateTransitionError
        );
    });

    it("Draft → Approved は禁止（段階スキップ）", () => {
        expect(() => assertClientTransition("Draft", "Approved")).toThrow(
            StateTransitionError
        );
    });

    it("Approved → Approved は禁止（同一状態）", () => {
        expect(() => assertClientTransition("Approved", "Approved")).toThrow(
            StateTransitionError
        );
    });
});
...
describe("L3: State Guard - StateTransitionError", () => {
    it("エラーメッセージに遷移元・遷移先が含まれる", () => {
        try {
            assertClientTransition("Approved", "Submitted");
        } catch (error) {
