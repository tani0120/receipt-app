        ...
        <div class="flex items-center justify-between mb-2">
             <div>
                 ...
                 <h2 class="text-xl font-bold text-slate-700 flex items-center gap-2">
                    <span class="bg-blue-100 text-blue-600 px-2 py-1 rounded text-sm font-mono">{{ ruleId }}</span>
                    {{ ruleName }}
                 </h2>
             </div>
             <button @click="showConfirmModal = true" class="bg-blue-600 text-white font-bold px-6 py-2 rounded shadow hover:bg-blue-700 transition flex items-center gap-2">
                 <i class="fa-solid fa-save"></i> 保存・修正
             </button>
        </div>

        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <label class="block text-sm font-bold text-gray-700 mb-2">ルール定義 (Prompt / Setting)</label>
            <textarea
                v-model="content"
                class="w-full h-96 border border-gray-300 rounded p-4 font-mono text-sm leading-relaxed focus:border-blue-500 outline-none resize-none bg-gray-50"
                placeholder="ここにルールやプロンプトを入力してください..."
            ></textarea>
        </div>
