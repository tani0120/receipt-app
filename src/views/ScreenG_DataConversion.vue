        ...
        <main class="flex-1 p-8 overflow-y-auto relative z-10">
            <div class="max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div class="flex flex-col gap-6">
                    ...
                    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                ...
                                <input v-model="form.clientName" type="text" placeholder="例: 株式会社サンプル商事"
                                    class="w-full bg-gray-50 border border-gray-200 rounded px-3 py-2 text-sm focus:outline-none focus:border-blue-500 transition">
                            </div>

                            <!-- Source Software -->
                            <div>
                                <label class="block text-xs font-bold text-gray-500 mb-1">移行前の会計ソフト名 <span class="text-xs font-normal text-gray-400">(自動判定可能)</span></label>
                                <div class="relative">
                                    <input v-model="form.sourceSoftware" type="text" placeholder="例: 弥生会計"
                                        class="w-full bg-gray-50 border border-gray-200 rounded px-3 py-2 text-sm focus:outline-none focus:border-blue-500 transition pr-8">
                    ...
                    <!-- Upload Area -->
                    <div
                        class="bg-white rounded-xl shadow-sm border-2 border-dashed p-8 text-center transition relative group overflow-hidden"
                        :class="[isDragging ? 'border-green-500 bg-green-50' : 'border-gray-200 hover:border-gray-300']"
                    ...
                    >
                        ...
                        <div v-if="!isProcessing && !uploadedFile" class="pointer-events-none">
                            ...
                            <button @click="(($refs.fileInput as HTMLInputElement).click())" class="pointer-events-auto bg-slate-800 text-white px-6 py-2 rounded-full text-xs font-bold hover:bg-slate-700 transition shadow hover:shadow-lg">
                                ファイルを選択
                            </button>
                        ...
                        <div v-if="uploadedFile && !isProcessing" class="animate-fade-in">
                            <div class="bg-blue-50 text-blue-700 p-4 rounded-lg flex items-center justify-between mb-4">
                                ...
                                <button @click="uploadedFile = null" class="text-blue-400 hover:text-blue-600">
                                    <i class="fa-solid fa-xmark"></i>
                                </button>
                            </div>
                            <button @click="startConversion" :disabled="!canConvert"
                                class="w-full py-3 rounded-lg font-bold text-sm transition shadow-lg flex items-center justify-center gap-2"
                                :class="canConvert ? 'bg-gradient-to-r from-green-500 to-emerald-600 text-white hover:shadow-green-500/30 hover:-translate-y-0.5' : 'bg-gray-200 text-gray-400 cursor-not-allowed'">
                                <i class="fa-solid fa-wand-magic-sparkles"></i> 変換を開始する
                            </button>
                        </div>
                ...
                <div class="flex flex-col h-full">
                    <div class="bg-white rounded-xl shadow-sm border border-gray-200 flex flex-col h-full relative overflow-hidden">
                        <div class="p-4 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
                            ...
                            <span class="text-[10px] bg-red-100 text-red-600 px-2 py-0.5 rounded border border-red-200 animate-pulse">
                                ★48時間以内に削除されます
                            </span>
                        </div>
                        ...
                        <div class="flex-1 overflow-y-auto p-4 space-y-3">
                            ...
                            <div v-for="log in logs" :key="log.id"
                                class="bg-white border border-gray-100 rounded-lg p-3 hover:shadow-md transition group animate-slide-in-right relative overflow-hidden">

                                <div class="flex justify-between items-start mb-2 pl-2">
                                    <div>
                                        <!-- DL Status Badge -->
                                        <div class="flex items-center gap-2 mb-1">
                                             <span v-if="!log.isDownloaded" class="bg-[#FDE047] text-[#854D0E] text-[10px] font-bold px-1.5 py-0.5 rounded animate-pulse">
                                                未DL
                                             </span>
                                             <span v-else class="bg-gray-100 text-gray-500 text-[10px] font-bold px-1.5 py-0.5 rounded">
                                                DL済
                                             </span>
                                             <span class="text-[10px] text-gray-400">{{ log.timestamp }}</span>
                                        </div>
                                        <div class="font-bold text-sm text-slate-800">{{ log.clientName }}</div>
                                    </div>
                                    <!-- Larger Font for Flow -->
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span class="text-xs font-bold px-2 py-1 rounded transition hover:opacity-80"
                                            :class="{
                                                'bg-yellow-100 text-yellow-700': log.targetSoftware === 'Yayoi',
                                                'bg-orange-100 text-orange-700': log.targetSoftware === 'MF',
                                                'bg-blue-100 text-blue-700': log.targetSoftware === 'Freee'
                                            }">
                                            {{ getSoftwareLabel(log.targetSoftware) }}
                                        </span>
                                    </div>
                                </div>

                                <div class="bg-slate-50 rounded p-2 pl-3 flex items-center justify-between group-hover:bg-slate-100 transition">
                                    ...
                                    <a :href="log.downloadUrl" :download="log.fileName" @click="handleDownload(log.id)"
                                        class="text-xs font-bold bg-white border border-gray-200 text-green-600 px-3 py-1.5 rounded flex items-center gap-1 hover:bg-green-50 hover:border-green-300 transition shadow-sm">
...
const processFile = (file: File) => {
    ...
    if (!form.value.sourceSoftware) {
        // Mock detection
        setTimeout(() => {
            form.value.sourceSoftware = '会計王'; // Mock detected value
        }, 500);
    }
};
...
const completeConversion = () => {
    ...
    // Add to global history via store
    addLog({
