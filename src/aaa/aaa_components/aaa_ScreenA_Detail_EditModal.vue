<template>
  <div v-if="visible" class="fixed inset-0 z-[9998] flex items-center justify-center bg-black/50 backdrop-blur-sm transition-opacity">
    ...
    <div class="bg-white rounded-lg shadow-xl w-[600px] max-h-[75vh] flex flex-col overflow-hidden animate-fade-in-up">
        ...
        <div class="px-6 py-4 bg-gray-50 border-t border-gray-100 flex justify-end gap-3 shrink-0">
            ...
            <button @click="save" :disabled="!isFormValid" class="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded shadow-md text-sm font-bold flex items-center gap-2 transition transform active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed">
                <i class="fa-solid fa-check"></i> 更新する
            </button>
        </div>
    </div>
  </div>
...
watch(() => props.visible, (newVal) => {
    if (newVal) {
        ...
        if (props.initialData) {
            ...
            // Map labels back to values if needed, or assume raw data matches
            if (d.consumptionTaxModeLabel === '原則課税') state.form.settings.taxMethod = 'inclusive'; // This mapping logic might be loose in Phase A
        }
    }
});
...
const save = () => {
    ...
    // Return mapped object compatible with ClientDetailUi/ClientUi if possible
    // For now returning the form state, parent handles re-mapping
