    ...
    <nav class="bg-white border-b border-gray-200 px-6 py-2 flex justify-between items-center shadow-sm z-50 shrink-0">
        <div class="flex items-center gap-6">
            ...
            <div class="flex space-x-1">
                <button @click="navigateTo('A')" :class="['px-3 py-2 text-xs font-bold rounded-t-lg transition flex items-center gap-2', isActive('/clients') ? 'bg-blue-50 text-blue-700 border-b-2 border-blue-500' : 'text-gray-500 hover:bg-gray-50']">
                    <i class="fa-solid fa-users"></i> A:顧問先管理
                </button>
                <button @click="navigateTo('B')" :class="['px-3 py-2 text-xs font-bold rounded-t-lg transition flex items-center gap-2', isActive('/journal-status') ? 'bg-blue-50 text-blue-700 border-b-2 border-blue-500' : 'text-gray-500 hover:bg-gray-50']">
                ...
                <button @click="navigateTo('H')" :class="['px-3 py-2 text-xs font-bold rounded-t-lg transition flex items-center gap-2 relative', isActive('/task-dashboard') ? 'bg-blue-50 text-blue-700 border-b-2 border-blue-500' : 'text-gray-500 hover:bg-gray-50']">
        ...
        <div class="flex items-center gap-4">
            ...
            <button @click="navigateToAdmin" class="text-xs font-bold px-3 py-1.5 rounded transition flex items-center gap-2 bg-white border text-slate-600 hover:bg-slate-100">
...
import { aaa_useAccountingSystem as useAccountingSystem } from '@/aaa/aaa_composables/aaa_useAccountingSystem';
import { seedRichJobs } from '@/aaa/aaa_utils/aaa_seedRichJobs';
import { aaa_useDataConversion as useDataConversion } from '@/aaa/aaa_composables/aaa_useDataConversion';
...
const navigateTo = (key: 'A' | 'B' | 'C' | 'D' | 'G' | 'H') => {
    router.push(getPath(key));
};

const isAaa = computed(() => route.path.startsWith('/aaa_'));

const getPath = (key: string) => {
    // Force aaa prefix for this layout
    switch (key) {
        case 'A': return '/aaa_clients';
        case 'B': return '/aaa_journal-status';
        case 'C': return '/aaa_collection-status';
        case 'D': return '/aaa_ai-rules';
        case 'G': return '/aaa_data-conversion';
        case 'H': return '/aaa_task-dashboard';
        default: return '/aaa_dashboard';
    }
};

const navigateToAdmin = () => {
    router.push('/aaa_admin-settings');
};
...
<style>
/* Global styles are imported in main.ts - shared for now but layout handles structure */
</style>
