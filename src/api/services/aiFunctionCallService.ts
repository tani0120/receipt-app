...
- 書き込み系コマンド（journal_write=仕訳投入、journal_cancel=仕訳取消、mf_sync_all=MF全データ同期）→ suggestionsで候補を返せ。直接実行するな。ツールとしても提供していない
...
- F1 操作ボタン提案: suggestionsで候補を返す（journal_write=仕訳投入、mf_sync_all=MF全データ同期など）
- F2 段階フロー:「①MF全データ同期(mf_sync_all)→②仕訳取得・確認(journal_view)→③仕訳投入(journal_write)の順で進めてください」
