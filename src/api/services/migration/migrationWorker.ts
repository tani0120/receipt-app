...
async function processOneJob(job: MigrationJob): Promise<void> {
  ...
  try {
    ...
    // 3. firstAi API呼び出し（excluded以外のみ）
    // 設計判断: excludedはAI分類不要でコスト削減。
    // 将来「仕訳外→仕訳対象に戻す」機能が必要な場合、
    // ステータス変更時に個別firstAiを実行する設計で対応する。
    let firstAiResult: FirstAiResponse | null = null;
    if (job.docStatus !== 'excluded') {
      try {
        const base64 = Buffer.from(preview.buffer).toString('base64');
        firstAiResult = await firstAiExtract({
          image: base64,
        ...
        console.log(
          `[migrationWorker] firstAi完了: ${job.driveFileId}`
          + ` → ${firstAiResult.source_type} (${firstAiResult.source_type_confidence})`
          + ` ${firstAiResult.line_items.length}行`
          + ` fallback=${firstAiResult.fallback_applied}`,
        );
      } catch (firstAiErr) {
        // firstAi失敗でもDL+Storageは続行（AI結果なしでジョブ完了）
        const msg = firstAiErr instanceof Error ? firstAiErr.message : String(firstAiErr);
        console.error(`[migrationWorker] firstAi失敗（続行）: ${job.driveFileId}: ${msg}`);
      }
    }
    ...
    console.log(
      ...
      + (firstAiResult ? ` ai=${firstAiResult.source_type}` : ' ai=skipped'),
    );
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    const elapsed = Date.now() - t0;
