  ...
  <div class="h-full flex flex-col bg-gray-50 font-sans" @click="closeDropdown">
    ...
    >
      ...
      <div class="flex items-center gap-2">
        ...
        <span class="text-gray-300">|</span>
        <button
          class="text-[13px] font-bold text-green-700 hover:text-green-900 flex items-center gap-0.5 transition-all"
          @click="handleExportMfCsv()"
          title="MFクラウド会計用CSVをダウンロード"
        >
          <i class="fa-solid fa-file-csv text-[10px]"></i>MF CSV出力
        </button>
        <span class="text-gray-300">|</span>
...
function handleExportMfCsv() {
  ...
  const allJournals = localJournals.value.filter((j: JournalPhase5Mock) => j.deleted_at === null);
  ...
  const excludedMsg =
    excluded.length > 0
      ? `\n\n※ ${excluded.length}件を除外:\n${excluded
          .slice(0, 5)
          .map((e) => `  - ${e.journal.description}（${e.reasons.join(", ")}）`)
