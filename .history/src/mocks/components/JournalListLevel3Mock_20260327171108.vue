...
import { validateForCsvExport, buildMfCsvContent, downloadMfCsv } from '../utils/exportMfCsv';
...
function handleExportMfCsv() {
  // 1. 出力対象の仕訳を取得（顧問先フィルタ済み、削除済み除外）
  const allJournals = localJournals.value
    .filter((j: JournalPhase5Mock) => j.deleted_at === null)

  if (allJournals.length === 0) {
    confirmDialog.value = {
      show: true,
      title: 'CSV出力',
      message: '出力対象の仕訳がありません。',
      showCancel: false,
      onConfirm: () => {},
    }
    return
  ...
  // 2. バリデーション（警告ラベル付き仕訳を除外）
  const { valid, excluded } = validateForCsvExport(allJournals)

  if (valid.length === 0) {
    confirmDialog.value = {
      show: true,
      title: 'CSV出力',
      message: `出力可能な仕訳がありません。\n除外: ${excluded.length}件（${excluded.map(e => e.reasons.join(', ')).join('; ')}）`,
      showCancel: false,
      onConfirm: () => {},
    }
    return
  ...
  // 3. 除外仕訳がある場合は確認ダイアログ
  const excludedMsg = excluded.length > 0
    ? `\n\n※ ${excluded.length}件を除外:\n${excluded.slice(0, 5).map(e => `  - ${e.journal.description}（${e.reasons.join(', ')}）`).join('\n')}${excluded.length > 5 ? `\n  ...他${excluded.length - 5}件` : ''}`
    : ''

  confirmDialog.value = {
    show: true,
    title: 'MFクラウドCSV出力',
    message: `${valid.length}件の仕訳をCSV出力します。${excludedMsg}`,
    showCancel: true,
    onConfirm: () => {
      // 4. CSV生成+ダウンロード
      const csvContent = buildMfCsvContent(valid, resolveAccountName, resolveTaxCategoryName)
      const cid = journalClientId.value || 'unknown'
      const today = new Date().toISOString().slice(0, 10).replace(/-/g, '')
      downloadMfCsv(csvContent, `mf_journal_${cid}_${today}.csv`)

      // ダウンロード完了通知
      confirmDialog.value = {
        show: true,
        title: 'CSV出力完了',
        message: `${valid.length}件のCSVをダウンロードしました。`,
        showCancel: false,
        onConfirm: () => {},
      }
    },
