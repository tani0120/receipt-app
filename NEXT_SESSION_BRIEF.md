...
### 技術戦略と実装詳細 (Technical Strategy & Details)

#### 1. アーキテクチャ採用理由 (Rationale)
*   ** GASの限界回避**: GASの6分の壁を回避するため、重い処理は全て **Firebase Cloud Functions (Node.js)** に委譲する。
*   ** 領域の分離**: 顧客はDriveのみ触る。システムはFirebaseで完結する。Driveは単なる「ポスト（投入口）」とみなす。
*   ** Tracer Bullet開発**: UI（ガワ）の作り込みを停止し、まずは「Drive -> Screen E」のデータ貫通を最優先する。

#### 2. 実装時の技術ポイント (Implementation Advice)
*   **A. Drive連携 (Ingest Logic)**:
    *   **GAS**: トリガー用途に限定。「ファイルID」と「会社ID」を通知するのみ。
    *   **Cloud Functions**: Google Drive APIを用いてストリームでファイルを直接Storageへバイパス転送する（メモリ対策）。
*   **B. Firestore構造 (Subcollection Pattern)**:
    *   構造: \companies/{companyId}/receipts/{receiptId}\`n    *   利点: セキュリティルールが書きやすく、Collection Group Queryで横断検索も可能。
*   **C. Batch APIフロー**: 
